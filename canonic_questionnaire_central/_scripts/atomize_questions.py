...
CLUSTER_MAPPING = {
    "PA02": "CL01",
    "PA05": "CL01",
    "PA08": "CL01",  # Seguridad y Paz
    "PA01": "CL02",
    "PA03": "CL02",
    "PA06": "CL02",
    "PA07": "CL02",  # Grupos Poblacionales
    "PA04": "CL03",  # Territorio y Ambiente
    "PA09": "CL04",
    "PA10": "CL04",  # Derechos Sociales y Crisis
}
...
class QuestionAtomizer:
    ...
    def _extract_mc_refs(self, dimension_id: str, expected_elements: List) -> List[str]:
        ...
        for ee in expected_elements:
            ...
            if any(
                kw in ee_type.lower()
                for kw in ["poblacion", "genero", "desagregacion", "diferencial"]
            ):
