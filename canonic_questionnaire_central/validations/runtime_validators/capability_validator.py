...
class CapabilityValidator:
    ...
    def _load_config(self, config_path: Optional[Path]) -> None:
        ...
        if config_path.exists():
            ...
            self._signal_capability_map = {
                ...
                "FINANCIAL_CHAIN": {
                    "required": ["NUMERIC_PARSING", "FINANCIAL_ANALYSIS"],
                    "optional": [],
                },
                "CAUSAL_LINK": {
                    "required": ["CAUSAL_INFERENCE", "GRAPH_CONSTRUCTION"],
                    "optional": [],
                },
                ...
                "SEMANTIC_RELATIONSHIP": {
                    "required": ["SEMANTIC_PROCESSING", "GRAPH_CONSTRUCTION"],
                    "optional": [],
                },
    ...
    def validate(
        self, consumer: SignalConsumerProtocol, signal_type: str
    ) -> CapabilityValidationResult:
        """Valida si un consumidor puede procesar un tipo de señal."""

        requirements = self._signal_capability_map.get(
            signal_type, {"required": [], "optional": []}
        )
    ...
    def can_process(self, consumer: SignalConsumerProtocol, signal_type: str) -> bool:
        """Shortcut para verificación rápida."""
        return self.validate(consumer, signal_type).can_process

    def get_processable_signal_types(self, consumer: SignalConsumerProtocol) -> Set[str]:
        ...
        for r in self._validation_log:
            if not r.can_process:
                ...
                failures_by_consumer[r.consumer_id].append(
                    {"signal_type": r.signal_type, "missing": list(r.missing_capabilities)}
                )
