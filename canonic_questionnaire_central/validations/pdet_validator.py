...
class PDETValidator:
    ...
    def __init__(
        self, pdet_data_path: Optional[Path] = None, templates_path: Optional[Path] = None
    ):
        """
    ...
    def _load_pdet_data(self, path: Path) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded PDET data: {self._pdet_data.get('overview', {}).get('total_municipalities', 0)} municipalities"
            )
    ...
    def _load_validation_templates(self, path: Path) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded validation templates version {self._validation_templates.get('version', 'unknown')}"
            )
        except Exception as e:
    ...
    def _build_indexes(self) -> None:
        ...
        logger.info(
            f"Indexed {len(self._municipality_index)} municipalities across {len(self._subregion_index)} subregions"
        )
    ...
    def get_pdet_validations_for_dimension(self, dimension: str) -> List[Dict[str, Any]]:
        ...
        pdet_validations = [v for v in dimension_templates if v.get("pdet_specific", False)]

        return pdet_validations

    def validate_pdet_context(
        self, dimension: str, municipality_code: str, validation_data: Dict[str, Any]
        ...
        for validation_rule in pdet_validations:
            result = self._apply_pdet_validation(
                validation_rule, dimension, pdet_context, validation_data
            )
    ...
    def _apply_pdet_validation(
        ...
        if validation_type == "pdet_municipality_context":
            self._validate_municipality_context(
                validation_rule, pdet_context, validation_data, result
            )
            ...
            self._validate_territorial_targeting(
                validation_rule, pdet_context, validation_data, result
            )
            ...
            self._validate_outcome_indicators(
                validation_rule, pdet_context, validation_data, result
            )
    ...
    def _validate_evidence_base(self, rule, context, data, result):
        ...
        if context.active_route_initiatives:
            completion_rate = (
                (context.active_route_initiatives / context.patr_initiatives * 100)
                if context.patr_initiatives
                else 0
            )
            result.recommendations.append(
...
def validate_pdet_context(
    dimension: str, municipality_code: str, validation_data: Dict[str, Any]
) -> List[PDETValidationResult]:
    """
