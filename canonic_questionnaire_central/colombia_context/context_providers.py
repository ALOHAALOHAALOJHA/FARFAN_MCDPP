...
class ContextProviderProtocol(Protocol):
    ...
    def load_context(self, context_type: str, filters: Optional[Dict[str, Any]] = None) -> EnrichmentContext:
...
class BaseContextProvider(ABC):
    ...
    def load_context(
        ...
        if context_type not in self.supported_contexts:
            raise ValueError(
                f"Context type '{context_type}' not supported by {self.provider_name}"
            )
...
class PDETContextProvider(BaseContextProvider):
    ...
    def _filter_by_policy_areas(
        ...
        # Filter subregions
        filtered_subregions = [
            sr for sr in raw_data.get("subregions", [])
        ...
        return [
            {
                ...
                "supported_contexts": p.supported_contexts
            }
