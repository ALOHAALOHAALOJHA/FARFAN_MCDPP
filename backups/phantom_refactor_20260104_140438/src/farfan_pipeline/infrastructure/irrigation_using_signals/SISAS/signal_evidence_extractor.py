...
def extract_structured_evidence(
    ...
    for element_spec in expected_elements:
        ...
        if isinstance(element_spec, str):
            ...
            element_type = element_spec.type if hasattr(element_spec, 'type') else element_spec.get('type', '')
...
def compute_completeness(
    ...
    for element_spec in expected_elements:
        element_type = element_spec.type if hasattr(element_spec, 'type') else element_spec.get('type', '')
        is_required = element_spec.required if hasattr(element_spec, 'required') else element_spec.get('required', False)
        minimum_count = element_spec.minimum if hasattr(element_spec, 'minimum') else element_spec.get('minimum', 0)
