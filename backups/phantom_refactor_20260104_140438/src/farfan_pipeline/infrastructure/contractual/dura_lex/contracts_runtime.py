...
class SemanticAnalyzerInputModel(StrictModel):
    ...
    segments: list[str] = Field(default_factory=list, description="Pre-segmented text chunks")
    ontology_params: dict[str, Any] = Field(
        default_factory=dict, description="Domain-specific ontology parameters"
    )
    schema_version: str = Field(
        default="sem-1.0", pattern=r"^sem-\d+\.\d+$", description="Contract schema version"
    )
...
class SemanticAnalyzerOutputModel(StrictModel):
    ...
    domain_classification: dict[str, float] = Field(description="Domain probability distribution")
...
class CDAFFrameworkInputModel(StrictModel):
    ...
    plan_metadata: dict[str, Any] = Field(
        default_factory=dict, description="Metadata about the plan"
    )
    config: dict[str, Any] = Field(default_factory=dict, description="Framework configuration")
    schema_version: str = Field(default="sem-1.0", pattern=r"^sem-\d+\.\d+$")
...
class CDAFFrameworkOutputModel(StrictModel):
    ...
    causal_mechanisms: list[dict[str, Any]] = Field(
        default_factory=list, description="Identified causal mechanisms"
    )
    ...
    bayesian_inference: dict[str, Any] = Field(
        default_factory=dict, description="Bayesian analysis results"
    )
    audit_results: dict[str, Any] = Field(
        default_factory=dict, description="Audit findings and recommendations"
    )
    schema_version: str = Field(default="sem-1.0", pattern=r"^sem-\d+\.\d+$")
...
class PDETAnalyzerOutputModel(StrictModel):
    ...
    extracted_tables: list[dict[str, Any]] = Field(
        default_factory=list, description="Extracted financial tables"
    )
    financial_indicators: dict[str, float] = Field(
        default_factory=dict, description="Calculated financial metrics"
    )
...
class TeoriaCambioInputModel(StrictModel):
    ...
    strategic_goals: list[str] = Field(
        default_factory=list, description="Identified strategic goals"
    )
    config: dict[str, Any] = Field(default_factory=dict, description="Configuration")
    schema_version: str = Field(default="sem-1.0", pattern=r"^sem-\d+\.\d+$")
...
class TeoriaCambioOutputModel(StrictModel):
    ...
    causal_dag: dict[str, Any] = Field(
        default_factory=dict, description="Causal directed acyclic graph"
    )
    validation_results: dict[str, Any] = Field(
        default_factory=dict, description="Model validation results"
    )
    monte_carlo_results: dict[str, Any] | None = Field(
        default=None, description="Monte Carlo simulation results"
    )
...
class ContradictionDetectorInputModel(StrictModel):
    ...
    dimension: str | None = Field(default=None, description="PolicyDimension enum value")
    config: dict[str, Any] = Field(default_factory=dict, description="Configuration")
    schema_version: str = Field(default="sem-1.0", pattern=r"^sem-\d+\.\d+$")
...
class ContradictionDetectorOutputModel(StrictModel):
    ...
    confidence_scores: dict[str, float] = Field(
        default_factory=dict, description="Confidence in each detection"
    )
    temporal_conflicts: list[dict[str, Any]] = Field(
        default_factory=list, description="Temporal inconsistencies"
    )
    severity_scores: dict[str, float] = Field(default_factory=dict, description="Severity ratings")
    schema_version: str = Field(default="sem-1.0", pattern=r"^sem-\d+\.\d+$")
...
class EmbeddingPolicyInputModel(StrictModel):
    ...
    dimensions: list[str] = Field(default_factory=list, description="Policy dimensions to analyze")
    embedding_model_config: dict[str, Any] = Field(
        default_factory=dict, description="Embedding model configuration", alias="model_config"
    )
    schema_version: str = Field(default="sem-1.0", pattern=r"^sem-\d+\.\d+$")
...
class EmbeddingPolicyOutputModel(StrictModel):
    ...
    similarity_scores: dict[str, float] = Field(
        default_factory=dict, description="Similarity metrics"
    )
    bayesian_evaluation: dict[str, Any] = Field(
        default_factory=dict, description="Bayesian evaluation results"
    )
    policy_metrics: dict[str, float] = Field(
        default_factory=dict, description="Policy-specific metrics"
    )
    schema_version: str = Field(default="sem-1.0", pattern=r"^sem-\d+\.\d+$")
...
class SemanticChunkingInputModel(StrictModel):
    ...
    preserve_structure: bool = Field(
        default=True, description="Whether to preserve document structure"
    )
    config: dict[str, Any] = Field(default_factory=dict, description="Configuration")
    schema_version: str = Field(default="sem-1.0", pattern=r"^sem-\d+\.\d+$")
...
class SemanticChunkingOutputModel(StrictModel):
    ...
    chunks: list[dict[str, Any]] = Field(default_factory=list, description="Semantic chunks")
    causal_dimensions: dict[str, dict[str, Any]] = Field(
        default_factory=dict, description="Causal dimension analysis"
    )
    key_excerpts: dict[str, list[str]] = Field(
        default_factory=dict, description="Key excerpts by category"
    )
...
class PolicyProcessorInputModel(StrictModel):
    ...
    sentences: list[str] = Field(default_factory=list, description="Pre-segmented sentences")
...
class PolicyProcessorOutputModel(StrictModel):
    ...
    processed_data: dict[str, Any] = Field(default_factory=dict, description="Processed results")
    evidence_bundles: list[dict[str, Any]] = Field(
        default_factory=list, description="Evidence bundles"
    )
    bayesian_scores: dict[str, float] = Field(default_factory=dict, description="Bayesian scores")
