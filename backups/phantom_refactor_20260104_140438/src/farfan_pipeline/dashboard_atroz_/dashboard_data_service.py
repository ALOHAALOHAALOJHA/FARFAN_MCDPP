...
class DashboardDataService:
    ...
    def summarize_region(
        ...
        summary = {
            ...
            "scores": scores,
            "clusterScores": {
                entry["cluster_id"]: entry["score_percent"]
    ...
    def _build_coordinates(self, record: dict[str, Any]) -> dict[str, float]:
        ...
        x = self._first_number(
            [coords.get("x"), coords.get("lng"), coords.get("lon")], default=50.0
        )
        y = self._first_number([coords.get("y"), coords.get("lat")], default=50.0)
        return {"x": float(x), "y": float(y)}

    def _build_metadata(
        self, record: dict[str, Any], macro_detail: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _build_indicators(
        ...
        cluster_map = {
            entry["cluster_id"].lower(): entry for entry in clusters if entry.get("cluster_id")
        }
    ...
    def _extract_macro_detail(
        self, report: dict[str, Any], record: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        band = (
            macro_section.get("quality_band")
            or macro_section.get("quality_level")
        ...
        coherence = self._first_number(
            [
                ...
                (
                    macro_section.get("metadata", {}).get("coherence")
                    if isinstance(macro_section.get("metadata"), dict)
                    else None
                ),
            ]
        )
    ...
    def _normalize_cluster_entry(
        ...
        coherence = self._first_number(
            [
                entry.get("coherence"),
                (
                    entry.get("metadata", {}).get("coherence")
                    if isinstance(entry.get("metadata"), dict)
                    else None
                ),
                (
                    entry.get("dispersion_metrics", {}).get("coherence")
                    if isinstance(entry.get("dispersion_metrics"), dict)
                    else None
                ),
            ]
        )
        variance = self._first_number(
            [
                ...
                (
                    entry.get("metadata", {}).get("variance")
                    if isinstance(entry.get("metadata"), dict)
                    else None
                ),
            ]
        ...
        if isinstance(area_scores, list):
            ...
            for area in area_scores:
                if not isinstance(area, dict):
                    continue
                candidate = (
                    area.get("policy_area_id") or area.get("policy_area") or area.get("name")
                )
                if candidate:
    ...
    def _extract_question_matrix(self, report: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if isinstance(micro_section, dict):
            if isinstance(micro_section.get("questions"), list):
                questions_raw.extend(
                    [item for item in micro_section["questions"] if isinstance(item, dict)]
                )
    ...
    def _normalize_question_entry(self, item: dict[str, Any]) -> dict[str, Any] | None:
        ...
        recommendations = self._normalize_recommendations(
            item.get("recommendations") or item.get("recommendation")
        )
    ...
    def _normalize_recommendation_entry(self, entry: Any) -> dict[str, Any] | None:
        if isinstance(entry, dict):
            ...
            category = str(
                entry.get("category") or entry.get("type") or entry.get("source") or "GENERAL"
            ).upper()
    ...
    def _first_number(
        self, candidates: Sequence[Any], default: float | None = None
    ) -> float | None:
