...
PHASE1_INPUT_PRECONDITIONS: List[Phase1InputPrecondition] = [
    ...
    Phase1InputPrecondition(
        "PRE-02", "PDF SHA256 must match provided hash", "validate_pdf_sha256", "CRITICAL"
    ),
    Phase1InputPrecondition(
        "PRE-03",
    ...
    Phase1InputPrecondition(
        "PRE-05", "Phase 0 validation must have passed", "validate_phase0_passed", "CRITICAL"
    ),
]
...
def validate_phase1_input_contract(canonical_input: Any) -> bool:
    ...
    if actual_hash != canonical_input.pdf_sha256:
        raise ValueError(
            f"PRE-02 failed: PDF SHA256 mismatch: expected {canonical_input.pdf_sha256}, got {actual_hash}"
        )

    # PRE-03: Questionnaire exists
    if not canonical_input.questionnaire_path.exists():
        raise ValueError(
            f"PRE-03 failed: Questionnaire does not exist: {canonical_input.questionnaire_path}"
        )
