...
Ubicación: src/farfan_pipeline/phases/Phase_two/contract_generator/chain_composer.py
...
class EpistemicChain:
    ...
    def get_methods_by_class(self, class_name: str) -> tuple:
        ...
        return tuple(m for m in self.full_chain_ordered if m.class_name == class_name)
...
class ChainComposer:
    ...
    def compose_chain(
        ...
        # ══════════════════════════════════════════════════════════════════
        expansion_context = self._build_expansion_context(method_set, contract_classification)
    ...
    def _validate_composed_chain(
        ...
        expected_total = (
            len(method_set.phase_a_N1) + len(method_set.phase_b_N2) + len(method_set.phase_c_N3)
        )
        ...
        for i, method in enumerate(chain.full_chain_ordered):
            if not method.method_id:
                raise RuntimeError(f"Method at index {i} has empty method_id")
    ...
    def _build_phase_metadata(self, phase_id: str, method_count: int) -> PhaseMetadata:
        """
