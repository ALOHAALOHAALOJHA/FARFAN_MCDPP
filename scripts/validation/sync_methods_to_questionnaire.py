...
def get_contract_file(repo_root: Path, q_index: int, pa_id: str) -> Path:
    ...
    return repo_root / 'src' / 'farfan_pipeline' / 'phases' / 'Phase_two' / 'generated_contracts' / f'Q{q_num}_{pa_id}_contract_v4.json'
...
def sync_methods(repo_root: Path, check_only: bool = False) -> tuple:
    ...
    for pa_id, folder in PA_FOLDERS.items():
        ...
        for i, q in enumerate(questions_data['questions']):
            ...
            current_methods = set(f"{m.get('class')}.{m.get('function')}" for m in q.get('method_sets', []))
            ...
            if current_methods != new_methods:
                ...
                if not check_only:
                    ...
                    questions_data['questions'][i]['method_sets_sync_timestamp'] = datetime.now(timezone.utc).isoformat()
                    modified = True
                    questions_updated += 1
