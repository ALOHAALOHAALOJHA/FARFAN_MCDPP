...
def load_questionnaire_sources(repo_root: Path) -> dict:
    ...
    for pa_id, folder in PA_FOLDERS.items():
        questions_file = repo_root / 'canonic_questionnaire_central' / 'policy_areas' / folder / 'questions.json'
...
def sync_contracts(repo_root: Path, check_only: bool = False) -> tuple:
    ...
    contracts_dir = repo_root / 'src' / 'farfan_pipeline' / 'phases' / 'Phase_two' / 'generated_contracts'
            ...
            if old_text != new_text:
                ...
                if not check_only:
                    ...
                    if 'expected_elements' in source_question:
                        contract['question_context']['expected_elements'] = source_question['expected_elements']
                    ...
                    if 'failure_contract' in source_question:
                        contract['question_context']['failure_contract'] = source_question['failure_contract']
                    
                    if 'question_id' in source_question:
                        contract['question_context']['source_question_id'] = source_question['question_id']
                    
                    contract['question_context']['sync_timestamp'] = datetime.now(timezone.utc).isoformat()
                    contract['question_context']['sync_source'] = f"canonic_questionnaire_central/policy_areas/{PA_FOLDERS[pa_id]}/questions.json"
