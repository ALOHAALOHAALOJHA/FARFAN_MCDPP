...
def validate_pdet_semantic_enrichment_file():
    ...
    try:
        ...
        if overview["total_municipalities"] != 170:
            print(f"❌ Overview: Expected 170 municipalities, found {overview['total_municipalities']}")
            return False
        ...
        for pa_id, pa_data in pa_mappings.items():
            required_fields = ["semantic_keywords", "pdet_relevance", "relevant_subregions", "key_indicators"]
            for field in required_fields:
        ...
        for pillar_id, pillar_data in pillars.items():
            required_fields = ["name", "semantic_keywords", "questionnaire_dimension", "policy_areas", "relevance_weight"]
            for field in required_fields:
        ...
        if not gate4.get("is_explicit") or not gate4.get("is_documented") or not gate4.get("is_traceable") or not gate4.get("is_governed"):
            print("❌ Gate 4 channel not fully compliant (missing explicit/documented/traceable/governed flags)")
            return False
...
def validate_semantic_patterns_have_pdet_context():
    ...
    try:
        ...
        if len(pdet_patterns) < 6:
            print(f"❌ Expected at least 6 PDET-specific patterns in territorial_patterns.json, found {len(pdet_patterns)}")
            return False
...
def validate_cross_reference_consistency():
    ...
    try:
        ...
        config_subregions = semantic_config["colombian_context_awareness"]["pdet_enrichment"]["total_subregions"]
        ...
        config_pa_count = len(semantic_config["colombian_context_awareness"]["pdet_enrichment"]["policy_area_mappings"])
        ...
        config_pillars_count = len(semantic_config["colombian_context_awareness"]["pdet_enrichment"]["pdet_pillars"])
...
def main():
    ...
    results.append(("Semantic Patterns PDET Context", validate_semantic_patterns_have_pdet_context()))
