...
class PatternRegistryExtractor:
        ...
        for q in questions:
            ...
            for p in patterns:
                ...
                if pid not in self.patterns:
                    ...
                    self.pattern_stats["by_category"][category] = self.pattern_stats["by_category"].get(category, 0) + 1
                    self.pattern_stats["by_specificity"][p.get('specificity')] = self.pattern_stats["by_specificity"].get(p.get('specificity'), 0) + 1
                    self.pattern_stats["by_match_type"][p.get('match_type')] = self.pattern_stats["by_match_type"].get(p.get('match_type'), 0) + 1
    ...
    def extract_all_patterns(self) -> None:
        ...
        print(f"  Total pattern-question associations: {sum(len(v) for v in self.pattern_to_questions.values())}")
    ...
    def analyze_pattern_usage(self) -> Dict[str, Any]:
        ...
        # Sort by usage
        sorted_usage = sorted(usage_analysis.items(), key=lambda x: x[1]["question_count"], reverse=True)
        ...
        registry = {
            ...
            "statistics": {
                **{k: v for k, v in self.pattern_stats.items() if k != "unique_regex_patterns"},
                ...
                "unique_regex_patterns_sample": list(self.pattern_stats["unique_regex_patterns"])[:100]
            },
            "patterns": {},
            "pattern_usage": {},
            ...
            registry["patterns"][pid] = {
                **pattern_data,
                "usage": {
                    ...
                    "question_ids": sorted(self.pattern_to_questions[pid], key=lambda x: int(x[1:]))
                }
            }

        # Add detailed usage analysis
        sorted_usage = sorted(
            self.pattern_to_questions.items(),
