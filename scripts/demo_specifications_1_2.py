...
def demonstrate_specification_2():
    ...
    territorial_contexts = create_territorial_contexts_from_enriched_results(
        enriched_results=enriched_results, policy_area="PA02"
    )
    ...
    adapter = TerritorialAggregationAdapter(
        enable_weight_adjustment=True, enable_dispersion_adjustment=True, strict_determinism=True
    )
    ...
    adjusted_weights, metadata = adapter.adjust_dimension_weights(
        base_weights=base_weights, territorial_contexts=territorial_contexts
    )
    ...
    for qid, weight in adjusted_weights.items():
        ...
        print(
            f"  {qid}: {weight:.3f} (multiplier={multiplier:.2f}, relevance={context.relevance.value if context else 'NONE'})"
        )
    ...
    scenario, penalty, meta = adapter.interpret_dispersion_with_context(
        coefficient_variation=cv, dispersion_index=di, territorial_contexts=territorial_contexts
    )
