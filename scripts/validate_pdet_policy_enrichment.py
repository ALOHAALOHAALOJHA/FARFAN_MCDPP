...
def validate_policy_area_enrichment(pa_dir: Path) -> Dict[str, Any]:
    ...
    gate1_valid = (
        gate1.get("required_scope") == "pdet_context" and
        "ENRICHMENT_DATA" in gate1.get("allowed_signal_types", []) and
        gate1.get("min_confidence", 0) >= 0.75
    )
    ...
    gate2_valid = (
        gate2.get("estimated_value_add", 0) >= 0.10 and
        len(gate2.get("enables", [])) > 0 and
        len(gate2.get("optimizes", [])) > 0
    )
    ...
    gate3_valid = (
        "SEMANTIC_PROCESSING" in required_capabilities and
        "TABLE_PARSING" in required_capabilities
    )
    ...
    gate4_valid = (
        gate4.get("is_explicit", False) and
        gate4.get("is_documented", False) and
        gate4.get("is_traceable", False) and
        gate4.get("is_governed", False) and
        gate4.get("source") == "colombia_context.pdet_municipalities"
    )
...
def main():
    ...
    pa_dirs = sorted([d for d in policy_areas_dir.iterdir() if d.is_dir() and d.name.startswith("PA")])
