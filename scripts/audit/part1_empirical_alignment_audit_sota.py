...
class SemanticAnalyzer:
    ...
    def search(
        self, query: str, text: str, threshold: float = 0.6, top_k: int = 5
        ...
        if self.cross_encoder:
            ...
            ranked_results = sorted(
                zip(cross_scores, top_results.indices), key=lambda x: x[0], reverse=True
            )
            ...
            for score, idx in ranked_results:
                ...
                if norm_score > 0.0:  # Tunable threshold for CrossEncoder
                    final_results.append(
                        (corpus[idx], min(1.0, max(0.0, (norm_score + 5) / 10)))
                    )  # Rough normalization
...
class NERAnalyzer:
    ...
    def _inject_domain_patterns(self):
        ...
        patterns = [
            ...
            {
                "label": "GPE",
                "pattern": [{"LOWER": "norte"}, {"LOWER": "de"}, {"LOWER": "santander"}],
            },
...
class PatternRegistry:
    ...
    def _load_sota_defaults(self):
        ...
        self._add_regex(
            "quant",
            [r"\b\d+[.,]?\d*\s*%\b", r"\b(tasa|índice|indicador|cobertura|tasa de)\b.*?\d+"],
        )
        self._add_regex(
            "territorial",
            [r"\b(municipio|vereda|corregimiento|cabecera|inspección|departamento)\b"],
        )
        self._add_regex(
            "institution", [r"\b(DANE|ICBF|DNP|MinSalud|MinEducación|MinHacienda|SGP)\b"]
        )
        self._add_regex("temporal", [r"\b(19\d{2}|20\d{2})\b", r"\b(año|período|vigencia)\b"])
        self._add_regex(
            "budget",
            [
                r"\b(presupuesto|inversión|recursos|apropiación|gasto)\b.*?\b[\d.]+\b",
                r"\b\$\s*[\d.]+\b",
            ],
        )
        self._add_regex(
            "causal",
            [
                r"\b(supuesto|riesgo|premisa)\b",
                r"\b(teoría\s+del\s+cambio)\b",
                r"\b(porque|debido a|dado que)\b",
            ],
        )
...
class IrrigationAuditor:
    ...
    def audit_element(
        self, element: ExpectedElement, text: str, question_text: str
    ) -> DetectionResult:
        ...
        if (element.weight > 0.5 and HAS_ST) or (hits < element.minimum_count and HAS_ST):
            ...
            sem_results = self.sem.search(
                query, text, threshold=0.1
            )  # Low threshold, cross-encoder will sort
        ...
        try:
            ...
            for q in data.get("questions", []):
                ...
                for el in q.get("expected_elements", []):
                    ...
                    expected_elements.append(ExpectedElement(et, req, min_c, cat, w))
...
def run_audit():
    ...
    for plan_name, plan_text in plan_iter:
        ...
        for contract in q_iter:
            ...
            weighted_score = sum(
                r.score * e.weight for r, e in zip(element_results, contract["expected_elements"])
            )

            missing_critical = any(
                not r.is_sufficient and e.required
                for r, e in zip(element_results, contract["expected_elements"])
            )
                ...
                plan_report["critical_gaps"].append(
                    {
                        ...
                        "dimension": dim,
                        "missing_elements": [
                            r.element_type for r in element_results if not r.is_sufficient
                        ],
                    }
                )
    ...
    if avg_alignment >= 0.7:
        ...
        md_lines.append(
            f"**VERDICT:** INSUFFICIENT. (Score: {avg_alignment:.2%}). Recommend enriching patterns."
        )
