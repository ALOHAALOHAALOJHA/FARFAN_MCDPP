...
ETHNIC_PATTERNS = {
    PDETSubregion.ALTO_PATIA: [
        ["Indígena Nasa", "Mestizo"],
    ...
    PDETSubregion.CHOCO: [
        ["Afrodescendiente", "Indígena"],
        ["Afrodescendiente", "Indígena", "Mestizo"],
    ],
...
def calculate_subregion_stats(
    municipalities: List[Dict[str, Any]], subregion_enum: PDETSubregion
) -> Dict[str, Any]:
    ...
    return {
        ...
        "dominant_sectors": sector_map.get(
            subregion_enum, ["Transporte", "Agricultura", "Desarrollo social"]
        ),
        "implementation_status": "En ejecución",
        "main_challenges": challenge_map.get(
            subregion_enum, ["Desarrollo rural", "Seguridad", "Conectividad"]
        ),
    }
...
def calculate_aggregate_statistics(all_municipalities: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    indigena_count = sum(
        1 for m in all_municipalities if any("Indígena" in e for e in m["ethnic_composition"])
    )
...
def main():
    ...
    for subregion_enum, subregion_id in SUBREGION_ID_MAP.items():
        ...
        policy_areas = [
            pa.replace("_", "") for pa, srs in POLICY_AREA_SUBREGIONS.items() if subregion_id in srs
        ]
    ...
    print(
        f"  - Total OCAD Paz investment: {enriched_data['aggregate_statistics']['financial_flows']['ocad_paz_total_cop_millions']:,} million COP"
    )

    # Validation checks
    print("\n=== Validation ===")
    subregion_counts = {
        sr["subregion_id"]: len(sr["municipalities"]) for sr in enriched_data["subregions"]
    }
    print("Municipalities per subregion:")
    for sr_id, count in sorted(subregion_counts.items()):
        sr_name = next(
            sr["name"] for sr in enriched_data["subregions"] if sr["subregion_id"] == sr_id
        )
