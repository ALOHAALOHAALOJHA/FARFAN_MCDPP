...
def execute_phase_1_with_full_contract(canonical_input: CanonicalInput) -> CanonPolicyPackage:
    ...
    AGGRESSIVELY PREVENTIVE MODE:
    - Pre-flight checks MUST pass before execution
    - All dependencies MUST be available
    - Resources MUST be sufficient
    - No graceful degradation - fail fast and loud
    """
    try:
        # SHOW CIRCUIT BREAKER STATUS
        circuit_breaker = get_circuit_breaker()
        if circuit_breaker.last_check:
            logger.info("Circuit Breaker Status: Previously validated")
        else:
            logger.info("Circuit Breaker Status: First run - will validate on execution")
