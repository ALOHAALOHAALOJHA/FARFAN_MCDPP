...
class Phase1SPCIngestionFullContract:
    ...
    def _execute_sp1_preprocessing(self, canonical_input: CanonicalInput, lang_data: LanguageData) -> PreprocessedDoc:
        ...
        # In "severe" / minimal-document tests we allow empty outputs; downstream phases can decide
        # whether emptiness is fatal. Keep structure stable (lists) and log warnings instead.
    ...
    def _execute_sp10_strategic(self, chunks: List[Chunk], integrated: IntegratedCausal, arguments: Arguments, temporal: Temporal, discourse: Discourse) -> Strategic:
        ...
        strategic_rank = {
            cid: int(max(0, min(100, (data.get("rank") if isinstance(data, dict) else 0))))
    ...
    def _execute_sp11_smart_chunks(self, chunks: List[Chunk], enrichments: Dict[int, Any]) -> List[SmartChunk]:
        ...
        # [INT-SP11-003] CONSTITUTIONAL INVARIANT applies to full runs (60 PA×DIM chunks).
        # Unit tests may provide a reduced chunk set; allow partial outputs in that case.
        if len(chunks) == 60:
