...
class IrrigationSynchronizer:
    ...
    def __init__(self, sisas_registry: SISASRegistry | None = None) -> None:
        """
    ...
    @precondition(
        lambda _self, chunks, _micro_answers: len(chunks) == CPP_CHUNK_COUNT,
        f"Input must contain exactly {CPP_CHUNK_COUNT} chunks"
    )
    @precondition(
        lambda _self, _chunks, micro_answers: len(micro_answers) == MICRO_ANSWER_COUNT,
    ...
    def _build_manifest(
        ...
        manifest_id = hashlib.sha256(manifest_content.encode()).hexdigest()
...
def synchronize_irrigation(
    chunks: list[CPPChunk],
    micro_answers: list[MicroAnswer],
    sisas_registry: SISASRegistry | None = None,
) -> SynchronizationManifest:
    """
