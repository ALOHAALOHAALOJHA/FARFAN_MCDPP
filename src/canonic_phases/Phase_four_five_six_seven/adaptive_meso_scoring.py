...
class AdaptiveMesoScoring:
    ...
    def compute_metrics(self, scores: list[float]) -> ScoringMetrics:
        ...
        if cv < self.config.convergence_cv_threshold and dispersion_index < self.config.convergence_di_threshold:
    ...
    def _compute_improvement_metric(
        ...
        penalty_difference = (1.0 - old_penalty_factor) - (1.0 - (adjusted_score / weighted_score if weighted_score > 0 else 0))
    ...
    def get_sensitivity_analysis(
        ...
        return {
            ...
            "sensitivity_to_perturbations": {
                ...
                "avg_abs_impact": sum(abs(p["impact"]) for p in perturbations) / len(perturbations) if perturbations else 0,
                "perturbations": perturbations[:5]  # Sample
            },
            "robustness_metrics": {
                "scenario_stability": "stable" if metrics.coefficient_variation < 0.3 else "unstable",
