...
class EnhancedClusterAggregator:
    ...
    def adaptive_penalty(self, dispersion: DispersionMetrics) -> float:
        ...
        if dispersion.scenario == "convergence":
            # Low dispersion: reduce penalty (0.5× multiplier)
            return base_penalty * 0.5
        elif dispersion.scenario == "moderate":
            # Moderate dispersion: standard penalty (1.0× multiplier)
            return base_penalty * 1.0
        elif dispersion.scenario == "high_dispersion":
            # High dispersion: increase penalty (1.5× multiplier)
            return base_penalty * 1.5
        else:  # extreme_dispersion
            # Extreme dispersion: maximum penalty (2.0× multiplier)
...
class EnhancedMacroAggregator:
    ...
    def compute_strategic_alignment(
        ...
        cluster_coherence_mean = sum(cluster_coherences) / len(cluster_coherences) if cluster_coherences else 0.0
        
        variance = sum((c - cluster_coherence_mean) ** 2 for c in cluster_coherences) / len(cluster_coherences) if len(cluster_coherences) > 1 else 0.0
        ...
        if len(cluster_scores_values) > 1:
            ...
            cluster_variance = sum((s - cluster_mean) ** 2 for s in cluster_scores_values) / len(cluster_scores_values)
...
def enhance_aggregator(aggregator: Any, level: str, enable_contracts: bool = True) -> Any:
    ...
    if level.lower() not in enhancers:
        raise ValueError(f"Invalid aggregation level: {level}. Must be one of {list(enhancers.keys())}")
