...
class SemanticProcessor:
    ...
    def _lazy_load(self) -> None:
        ...
        try:
            ...
            self._model = AutoModel.from_pretrained(
                ...
                torch_dtype=(
                    torch.float16 if self.config.fp16 and device == "cuda" else torch.float32
                ),
            ).to(device)
            self._model.eval()
            self._loaded = True
    ...
    def chunk_text(self, text: str, preserve_structure: bool = True) -> list[dict[str, Any]]:
        ...
        for section in sections:
            ...
            tokens = self._tokenizer.encode(
                section["text"], add_special_tokens=False, truncation=False
            )
    ...
    def _detect_pdm_structure(self, text: str) -> list[dict[str, Any]]:
        ...
        header_patterns = [
            re.compile(p) for p in UnitOfAnalysisLoader.get_patterns("section_headers")
        ]
        ...
        for idx, header in enumerate(ordered_headers):
            start = int(header["end"])
                ...
                int(ordered_headers[idx + 1]["start"])
                if idx + 1 < len(ordered_headers)
                else len(text)
            )
...
class BayesianEvidenceIntegrator:
    ...
    def integrate_evidence(
        self, similarities: NDArray[np.float64], chunk_metadata: list[dict[str, Any]]
    ) -> dict[str, float]:
        """
        ...
        posterior_variance = (alpha_pos * alpha_neg) / (alpha_total**2 * (alpha_total + 1))
...
class PolicyDocumentAnalyzer:
    ...
    def analyze(self, text: str) -> dict[str, Any]:
        ...
        for dim, dim_emb in self.dimension_embeddings.items():
            similarities = np.array([1.0 - cosine(chunk["embedding"], dim_emb) for chunk in chunks])
            # Filter by threshold
            ...
            if len(relevant_sims) >= self.config.min_evidence_chunks:
                evidence = self.bayesian.integrate_evidence(relevant_sims, relevant_chunks)
            else:
    ...
    def _extract_key_excerpts(
        self, chunks: list[dict[str, Any]], dimension_results: dict[str, dict[str, Any]]
    ) -> dict[str, list[str]]:
        ...
        for dim, dim_emb in self.dimension_embeddings.items():
            ...
            sims = [
                (i, 1.0 - cosine(chunk["embedding"], dim_emb)) for i, chunk in enumerate(chunks)
            ]
            sims.sort(key=lambda x: x[1], reverse=True)
            # Top 3 excerpts
            top_chunks = [chunks[i] for i, _ in sims[:3]]
            excerpts[dim.value] = [
                _get_chunk_content(c)[:300] + ("..." if len(_get_chunk_content(c)) > 300 else "")
                for c in top_chunks
            ]
        return excerpts
...
class SemanticChunkingProducer:
    ...
    def get_dimension_analysis(
        self, analysis: dict[str, Any], dimension: CausalDimension
    ) -> dict[str, Any]:
        """Extract specific dimension results from analysis"""
        return analysis.get("causal_dimensions", {}).get(dimension.value, {})

    def get_dimension_score(self, analysis: dict[str, Any], dimension: CausalDimension) -> float:
...
"""
    config = SemanticConfig(
        chunk_size=512, chunk_overlap=100, similarity_threshold=0.8  # Refactored
    )
