...
class PDETMunicipalPlanAnalyzer:
    ...
    def _extract_from_budget_table(self, df: pd.DataFrame) -> list[FinancialIndicator]:
        ...
        amount_cols = [
            col
            for col in df.columns
            if any(kw in str(col).lower() for kw in ["monto", "valor", "presupuesto", "recursos"])
        ]
        source_cols = [
            col
            for col in df.columns
            if any(kw in str(col).lower() for kw in ["fuente", "financiación", "origen"])
        ]
    ...
    def _assess_financial_sustainability(
        ...
        diversity_score = min(
            funding_sources.get("diversity_index", 0) / funding_sources.get("max_diversity", 1), 1.0
        )

        distribution = funding_sources.get("distribution", {})
        total = sum(distribution.values())
        own_resources_amount = distribution.get("Recursos Municipales", 0) + distribution.get(
            "Recursos Propios", 0
        )
    ...
    def _extract_from_responsibility_tables(
        ...
        for table in resp_tables:
            ...
            resp_cols = [
                col
                for col in df.columns
                if any(kw in str(col).lower() for kw in ["responsable", "ejecutor", "encargado"])
            ]
    ...
    def _score_entity_specificity(
        ...
        for entity in entities:
            ...
            final_score = (
                length_score * 0.2 + propn_score * 0.3 + inst_score * 0.3 + mention_score * 0.2
            )
    ...
    def construct_causal_dag(
        self, text: str, tables: list[ExtractedTable], financial_analysis: dict[str, Any]
    ) -> CausalDAG:
    ...
    def _find_semantic_mentions(
        self, text: str, concept: str, concept_embedding: np.ndarray
    ) -> list[str]:
    ...
    def _extract_budget_for_pillar(
        self, pillar: str, text: str, financial_analysis: dict[str, Any]
    ) -> Decimal | None:
        ...
        for pattern, edge_type in causal_patterns:
            for match in re.finditer(pattern, text[:30000], re.IGNORECASE):
                ...
                if source_node and target_node and source_node != target_node:
                    existing = next(
                        (e for e in edges if e.source == source_node and e.target == target_node),
                        None,
                    )
                    ...
                    if existing:
                        existing.probability = min(
                            ...
                            existing.probability + PROBABILITY_REINFORCEMENT,
                        )
    ...
    def _refine_edge_probabilities(
        self, edges: list[CausalEdge], text: str, nodes: dict[str, CausalNode]
    ) -> list[CausalEdge]:
        text_lower = text.lower()
        ...
        for edge in edges:
            ...
            positions_source = [
                m.start() for m in re.finditer(re.escape(edge.source[:30].lower()), text_lower)
            ]
            positions_target = [
                m.start() for m in re.finditer(re.escape(edge.target[:30].lower()), text_lower)
            ]
    ...
    def estimate_causal_effects(
        self, dag: CausalDAG, text: str, financial_analysis: dict[str, Any]
    ) -> list[CausalEffect]:
    ...
    def _estimate_effect_bayesian(
        self, treatment: str, outcome: str, dag: CausalDAG, financial_analysis: dict[str, Any]
    ) -> CausalEffect | None:
        G = dag.graph
        try:
        ...
        budget_value = (
            float(treatment_node.associated_budget) if treatment_node.associated_budget else 0.0
        )
        ...
        with pm.Model():
            ...
            for path in indirect_paths[:3]:
                ...
                indirect_eff = pm.Normal(
                    f"indirect_{path_name}", mu=prior_mean * 0.5, sigma=prior_sd * 1.5
                )
                indirect_effects.append(indirect_eff)

            if budget_value > 0:
                budget_adjustment = pm.Deterministic(
                    "budget_adjustment", pm.math.log1p(budget_value / 1e9)
                )
            ...
            if indirect_effects:
                total_effect = pm.Deterministic(
                    "total_effect", adjusted_direct + pm.math.sum(indirect_effects)
                )
            else:
                total_effect = pm.Deterministic("total_effect", adjusted_direct)

            evidence_strength = (
                treatment_node.evidence_strength * dag.nodes[outcome].evidence_strength
            )
    ...
    def generate_counterfactuals(
        self, dag: CausalDAG, causal_effects: list[CausalEffect], financial_analysis: dict[str, Any]
        ...
        current_budgets = {
            node: (
                float(dag.nodes[node].associated_budget)
                if dag.nodes[node].associated_budget
                else 0.0
            )
        ...
        scenario_1 = self._simulate_intervention(
            intervention_1, dag, causal_effects, "Incremento 20% presupuesto"
        )
        ...
        scenario_2 = self._simulate_intervention(
            intervention_2, dag, causal_effects, "Priorización educación y salud"
        )
        ...
        if causal_effects:
            best_effect = max(
                causal_effects, key=lambda e: e.probability_positive * abs(e.posterior_mean)
            )
            ...
            scenario_3 = self._simulate_intervention(
                intervention_3, dag, causal_effects, f"Focalización en {best_pillar[:40]}"
            )
    ...
    def _simulate_intervention(
        ...
        for outcome in outcome_nodes:
            ...
            for effect in relevant_effects:
                ...
                current_budget = (
                    float(dag.nodes[treatment].associated_budget)
                    if dag.nodes[treatment].associated_budget
                    else 0.0
                )
        ...
        narrative = self._generate_scenario_narrative(
            description, intervention, predicted_outcomes, probability_improvement
        )
    ...
    def sensitivity_analysis(
        self, causal_effects: list[CausalEffect], dag: CausalDAG
    ) -> dict[str, Any]:
        """
        ...
        for effect in causal_effects[:10]:  # Top 10 effects
            ...
            sensitivity_results[f"{effect.treatment[:30]}→{effect.outcome[:30]}"] = {
                ...
                "interpretation": self._interpret_sensitivity(
                    unobserved_confounding, robustness_value
                ),
            }
    ...
    def _score_indicators(self, tables: list[ExtractedTable], text: str) -> float:
        ...
        for table in indicator_tables:
            ...
            present_cols = sum(
                1 for col in required_cols if any(col in str(c).lower() for c in df.columns)
            )
        ...
        smart_count = sum(
            len(re.findall(pattern, text, re.IGNORECASE)) for pattern in smart_patterns
        )
        smart_score = min(smart_count / 50, 1.0) * 3.0

        formula_mentions = len(re.findall(r"f[óo]rmula", text, re.IGNORECASE))
        periodicity_mentions = len(
            re.findall(r"periodicidad|trimestral|anual|mensual", text, re.IGNORECASE)
        )
    ...
    def _score_responsibility_clarity(self, entities: list[ResponsibleEntity]) -> float:
        ...
        institutional_entities = [
            e for e in entities if e.entity_type in ["secretaría", "dirección", "oficina"]
        ]
    ...
    def _score_temporal_consistency(self, text: str, tables: list[ExtractedTable]) -> float:
        ...
        term_count = sum(
            len(re.findall(rf"\b{term}\b", text, re.IGNORECASE)) for term in temporal_terms
        )
        term_score = min(term_count / 30, 1.0) * 3.0

        return float(min(range_score + cronograma_score + term_score, 10.0))

    def _score_pdet_alignment(
        self, text: str, tables: list[ExtractedTable], dag: CausalDAG
    ) -> float:
    ...
    def _score_causal_coherence(self, dag: CausalDAG, effects: list[CausalEffect]) -> float:
        ...
        connected_pillars = sum(
            1 for p in pillar_nodes if any(nx.has_path(G, p, o) for o in outcome_nodes)
        )
    ...
    def _estimate_score_confidence(
    ...
    def generate_executive_report(self, analysis_results: dict[str, Any]) -> str:
        ...
        if fin["funding_sources"] and fin["funding_sources"]["distribution"]:
            for source, amount in sorted(
                fin["funding_sources"]["distribution"].items(), key=lambda x: -x[1]
            )[:5]:
    ...
    def _generate_recommendations(self, analysis_results: dict[str, Any]) -> str:
        ...
        scenarios = analysis_results.get("counterfactuals", [])
        if scenarios:
            best_scenario = max(scenarios, key=lambda s: sum(s["probability_improvement"].values()))
    ...
    def analyze_municipal_plan_sync(
    ...
    async def analyze_municipal_plan(
        ...
        counterfactuals = self.generate_counterfactuals(
            causal_dag, causal_effects, financial_analysis
        )
        ...
        quality_score = self.calculate_quality_score(
            full_text, tables, financial_analysis, responsible_entities, causal_dag, causal_effects
        )
        ...
        results = {
            ...
            "extraction": {
                ...
                "table_types": {
                    t.table_type: sum(1 for x in tables if x.table_type == t.table_type)
                    for t in tables
                    if t.table_type
                },
            },
            ...
            "causal_dag": {
                ...
                "pillar_nodes": [
                    n for n, node in causal_dag.nodes.items() if node.node_type == "pilar"
                ],
                "outcome_nodes": [
                    n for n, node in causal_dag.nodes.items() if node.node_type == "outcome"
                ],
            },
        ...
        for gap in gaps:
            ...
            if gap_type == "missing_baseline":
                remediation["recommendation"] = (
                    "Establecer línea base cuantitativa basada en diagnóstico actual"
                )
            elif gap_type == "missing_target":
                remediation["recommendation"] = (
                    "Definir meta cuantitativa con horizonte temporal claro"
                )
                ...
                remediation["recommendation"] = (
                    "Asignar presupuesto específico con fuente de financiación"
                )
    ...
    def generate_recommendations(self, analysis_results: dict[str, Any]) -> list[str]:
        ...
        if analysis_results.get("temporal_consistency", 0) < 0.7:
            recommendations.append("Establecer cronograma claro con hitos y plazos definidos")
...
async def main_example() -> None:
    ...
    analyzer = PDETMunicipalPlanAnalyzer(
        use_gpu=True, language="es", confidence_threshold=0.7  # Refactored
    )
    ...
    try:
        ...
        results = await analyzer.analyze_municipal_plan(
            pdf_path=pdf_path, output_dir="outputs/analisis_pdm"
        )
...
class FinancialAggregator:
    ...
    def aggregate_financial_data(
        ...
        if population and population > 0:
            per_capita_values = self.normalize_to_population(
                [e.get("amount", 0) for e in entries], population
            )
        ...
        for i, entry in enumerate(entries):
            normalized_fact = {
                ...
                "percentage_of_budget": (
                    budget_percentages[i] if i < len(budget_percentages) else 0.0
                ),
    ...
    def normalize_to_budget_base(self, amounts: list[float], total_budget: float) -> list[float]:
        """
    ...
    def normalize_to_population(self, amounts: list[float], population: int) -> dict[int, float]:
        """
        ...
        return {i: amt / population for i, amt in enumerate(amounts)}
...
def _run_quality_gates() -> dict[str, bool]:
    ...
    results["micro_levels_monotonic"] = all(
    ...
    results["funding_sources_min"] = set(["SGP", "SGR", "Recursos Municipales"]).issubset(
        set(FUNDING_SOURCE_KEYWORDS.keys())
    )

    # Causal priors ordered
    results["causal_priors_ordered"] = (
        PROBABILITY_PRIORS["direct"]
        >= PROBABILITY_PRIORS["mediated"]
