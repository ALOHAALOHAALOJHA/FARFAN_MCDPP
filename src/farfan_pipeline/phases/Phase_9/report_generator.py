...
class ReportGenerator:
        ...
        try:
            ...
            if self.enable_charts:
                chart_paths = generate_charts(
                    report, output_dir=self.output_dir, plan_name=self.plan_name
                )
            ...
            if generate_html:
                ...
                html_content = generate_html_report(report, chart_paths=chart_paths)
    ...
    def _generate_manifest(
        self, artifacts: dict[str, Path], report: AnalysisReport
    ) -> dict[str, Any]:
...
def generate_markdown_report(report: AnalysisReport) -> str:
    ...
    if report.macro_summary and report.macro_summary.recommendations:
        ...
        for i, rec in enumerate(report.macro_summary.recommendations, 1):
            icon = (
                ...
                else "🟠" if rec.severity == "HIGH" else "🟡" if rec.severity == "MEDIUM" else "🔵"
            )
            lines.extend(
                [
    ...
    for i, analysis in enumerate(report.micro_analyses[:10], 1):
        ...
        lines.append(
            f"**Dimensión:** {dimension} | **Área de Política:** {policy_area} | **Puntuación:** {score_str}\n"
        )
    ...
    if len(report.micro_analyses) > 10:
        ...
        for analysis in report.micro_analyses[10:30]:  # Show next 20 in table
            dimension = analysis.metadata.get("dimension", "N/A") if analysis.metadata else "N/A"
            policy_area = (
                analysis.metadata.get("policy_area", "N/A") if analysis.metadata else "N/A"
            )
            score_str = f"{analysis.score:.4f}" if analysis.score is not None else "N/A"
...
def generate_html_report(report: AnalysisReport, chart_paths: list[Path] | None = None) -> str:
    ...
    try:
        ...
        raise ReportGenerationError("Jinja2 not installed. Run: pip install jinja2") from e

    # Get template directory
    template_dir = Path(__file__).resolve().parent / "templates"

    if not template_dir.exists():
        raise ReportGenerationError(f"Template directory not found: {template_dir}")
...
def generate_charts(
    report: AnalysisReport, output_dir: Path, plan_name: str = "plan1"
) -> list[Path]:
