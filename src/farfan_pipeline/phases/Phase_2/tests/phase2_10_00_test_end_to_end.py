...
class TestContractLoadingEndToEnd:
    ...
    def test_all_contracts_load_as_valid_json(self, sample_contracts: list[Path]) -> None:
        ...
        assert not failures, f"INVALID JSON in {len(failures)} contracts:\n" + "\n".join(
            f"  {name}: {err}" for name, err in failures[:10]
        )
...
class TestEvidenceAssemblyEndToEnd:
    ...
    def test_evidence_assembly_module_has_process_evidence(self) -> None:
        ...
        assert (
            "def process_evidence" in content
...
class TestCarverSynthesisEndToEnd:
    ...
    def test_carver_enforces_citations(self) -> None:
        ...
        assert (
            "citation" in content.lower() or "reference" in content.lower()
...
class TestOutputSchemaCompliance:
    ...
    def test_phase2_result_has_required_fields(self) -> None:
        ...
        missing = [
            f for f in required_result_fields if f'"{f}"' not in content and f"'{f}'" not in content
        ]

        # Allow for some flexibility in naming
        assert len(missing) < 3, f"Phase2QuestionResult may be missing fields: {missing}"
    ...
    def test_evidence_graph_format_defined(self) -> None:
        ...
        assert (
            "EvidenceGraph" in content or "evidence_graph" in content
...
class TestErrorHandlingEndToEnd:
    ...
    def test_contract_has_error_handling_section(self) -> None:
        ...
        has_error_handling = "error_handling" in contract or "failure_contract" in question_context

        assert has_error_handling, "Contract MUST have error_handling or failure_contract section."
    ...
    def test_base_executor_has_error_handling(self) -> None:
        ...
        assert (
            "_check_failure_contract" in content
