...
CONTRACT_DOMINANT_CLASSES = {
    ...
    ContractType.TYPE_D: ["FinancialAuditor", "PDETMunicipalPlanAnalyzer", "FinancialAggregator"],
    ContractType.TYPE_E: [
        "PolicyContradictionDetector",
        "IndustrialGradeValidator",
        "OperationalizationAuditor",
...
RAW_METHOD_INVENTORY = {
    "analyzer_one.py": {
        ...
        "SemanticAnalyzer": [
            "__init__",
            "_load_unit_of_analysis_stats",
            "_compute_unit_of_analysis_natural_blocks",
            ...
            "_get_slot_threshold",
            "_keyword_score",
            "_score_d3_q3_expected_elements",
            "_score_base_slots",
        ...
        "DocumentProcessor": [
            ...
            "load_docx",
            "segment_text",
            "load_canonical_question_contracts",
            "segment_by_canonical_questionnaire",
            "_default_policy_area_id",
            "_to_canonical_dimension_id",
        ],
    ...
    "bayesian_multilevel_system.py": {
        ...
        "BayesianEvidenceExtractor": [
            "__init__",
            "extract_prior_beliefs",
            "extract_likelihood_evidence",
            "extract_statistical_metadata",
            "_extract_probability",
            "_extract_likelihood_value",
            "_extract_measurement_unit",
    ...
    "contradiction_deteccion.py": {
        ...
        "SemanticValidator": [
            ...
            "_check_quantitative_data_presence",
            "_check_baseline_indicator",
        ...
        "ContradictionDominator": [
            "__init__",
            "apply_dominance_veto",
            "detect_any_contradiction",
            "_detect_any_contradiction_from_dicts",
            "generate_veto_report",
            "_generate_veto_report_from_facts",
            "_calculate_original_confidence_from_dicts",
        ],
        "DempsterShaferCombinator": [
            "__init__",
            "combine_belief_masses",
            "calculate_conflict_mass",
            "normalize_belief_distribution",
            "_dempster_combination",
        ...
        "LogicalConsistencyChecker": [
            ...
            "enforce_no_averaging_prohibition",
            "_enforce_no_averaging_prohibition",
        ],
    },
    "derek_beach.py": {
        ...
        "BayesianMechanismInference": [
            ...
            "_extract_dimension_evidence",
            "_calculate_likelihood_from_evidence",
            "_bayesian_update",
            "_infer_chain_capacity_vector",
        ...
        "CDAFFramework": [
            "__init__",
            "process_document",
            "_generate_bayesian_reports",
            "_verify_cvc_compliance",
        ...
        "DerekBeachProducer": [
            ...
            "get_r_hat",
            "get_ess",
            "is_inference_uncertain",
            "posterior_predictive_check",
            "get_ppd_p_value",
            "get_ablation_curve",
            "get_ppc_recommendation",
            "verify_conditional_independence",
            "get_independence_tests",
            "get_delta_waic",
    ...
    "embedding_policy.py": {
        ...
        "EmbeddingPolicyProducer": [
            ...
            "get_chunk_embedding",
            "get_chunk_metadata",
            "get_chunk_pdq_context",
            "semantic_search",
            "get_search_result_chunk",
            "get_search_result_score",
            "generate_pdq_report",
            "get_pdq_evidence_count",
            "get_pdq_numerical_evaluation",
            "get_pdq_evidence_passages",
            "get_pdq_confidence",
            "evaluate_numerical_consistency",
            "get_point_estimate",
            "get_credible_interval",
            "get_evidence_strength",
            "get_numerical_coherence",
            "compare_policy_interventions",
            "get_comparison_probability",
            "get_comparison_bayes_factor",
            "get_comparison_difference_mean",
            "get_diagnostics",
            "get_config",
            "list_policy_domains",
            "list_analytical_dimensions",
            "get_policy_domain_description",
            "get_analytical_dimension_description",
    ...
    "financiero_viabilidad_tablas.py": {
        ...
        "FinancialAggregator": [
            ...
            "normalize_to_budget_base",
            "normalize_to_population",
    ...
    "policy_processor.py": {
        ...
        "IndustrialPolicyProcessor": [
            ...
            "_build_canonical_point_patterns",
            "_detect_policy_areas",
            "_detect_scoring_modality",
    ...
    "teoria_cambio.py": {
        "TeoriaCambio": [
            ...
            "construir_grafo_from_cpp",
            "construir_grafo_from_spc",
            "validacion_completa",
            "_extraer_categorias",
        ...
        "DAGCycleDetector": [
            ...
            "detect_cycles",
            "veto_on_cycle",
            "calculate_acyclicity_confidence",
...
class EpistemologicalClassifier:
    ...
    def __init__(self):
        ...
        self.classification_stats = dict.fromkeys(EpistemologicalLevel, 0)
    ...
    def _determine_level(
    ...
    ) -> tuple[EpistemologicalLevel, str, float]:
        """
        ...
        if method_name in ["__init__", "__post_init__", "__str__", "__repr__"]:
            return (EpistemologicalLevel.N0_INFRA, "Método de inicialización/infraestructura", 0.95)
        ...
        for level, patterns in LEVEL_PATTERNS.items():
            if class_name in patterns.get("classes", []):
                ...
                return (level, f"Clase {class_name} es dominante en {level.code}", 0.85)
        ...
        for level, patterns in LEVEL_PATTERNS.items():
            ...
            for keyword in patterns.get("keywords", []):
                if keyword.lower() in method_name.lower() or keyword.lower() in class_name.lower():
                    return (level, f"Keyword '{keyword}' detectado → {level.code}", 0.80)
        ...
        if any(kw in method_lower for kw in ["veto", "block", "suppress", "reject", "invalid"]):
            return (
                EpistemologicalLevel.N3_AUD,
                "Semántica de veto/bloqueo → N3-AUD (VETO GATE)",
                0.85,
            )
        ...
        if any(
            kw in method_lower
            for kw in ["posterior", "prior", "likelihood", "bayes", "score", "confidence"]
        ):
            return (EpistemologicalLevel.N2_INF, "Semántica bayesiana/inferencial → N2-INF", 0.82)

        # N1: Patrones de extracción
        if any(kw in method_lower for kw in ["get", "load", "read", "fetch", "find"]):
            return (EpistemologicalLevel.N1_EMP, "Semántica de obtención de datos → N1-EMP", 0.75)
        ...
        if any(kw in method_lower for kw in ["report", "summary", "export", "generate"]):
            if "recommendation" in method_lower or "failure" in method_lower:
                return (EpistemologicalLevel.N4_META, "Semántica de meta-análisis → N4-META", 0.78)

        # Default: Inferir por contexto de clase
        if "Validator" in class_name or "Auditor" in class_name or "Detector" in class_name:
            return (
                EpistemologicalLevel.N3_AUD,
                f"Clase {class_name} sugiere auditoría → N3-AUD",
                0.70,
            )
        ...
        if "Bayesian" in class_name or "Inference" in class_name:
            return (
                EpistemologicalLevel.N2_INF,
                f"Clase {class_name} sugiere inferencia → N2-INF",
                0.70,
            )
        ...
        if "Extract" in class_name or "Processor" in class_name:
            return (
                EpistemologicalLevel.N1_EMP,
                f"Clase {class_name} sugiere extracción → N1-EMP",
                0.70,
            )
        ...
        return (
            EpistemologicalLevel.N2_INF,
            "Clasificación por defecto (requiere revisión manual)",
            0.50,
        )
    ...
    def _calculate_contract_affinities(
        self, class_name: str, method_name: str, level: EpistemologicalLevel
    ) -> dict[str, float]:
        """
    ...
    def _level_contract_affinity(
        self, level: EpistemologicalLevel, contract_type: ContractType
    ) -> float:
...
class MethodSetGenerator:
    ...
    def _select_phase_methods(
        self, level: EpistemologicalLevel, contract_type: ContractType, top_k: int
        ...
        sorted_methods = sorted(
            level_methods,
            key=lambda m: (m.contract_affinities.get(contract_type.code, 0), m.confidence_score),
            reverse=True,
        )
    ...
    def _calculate_balance_score(
        self, n1: int, n2: int, n3: int, contract_type: ContractType
    ) -> float:
    ...
    def _generate_doctoral_justification(
        self,
        question_id: str,
        contract_type: ContractType,
        phase_a: list[ClassifiedMethod],
        ...
        if phase_b:
            ...
            avg_affinity = sum(
                m.contract_affinities.get(contract_type.code, 0) for m in phase_b
            ) / len(phase_b)
            p3 = (
...
class ResultsExporter:
    ...
    @staticmethod
    def export_classified_methods(methods: list[ClassifiedMethod], output_path: Path) -> None:
    ...
    @staticmethod
    def export_method_sets(method_sets: dict[str, MethodSet], output_path: Path) -> None:
...
def main():
    ...
    ResultsExporter.export_classified_methods(
        classified_methods, output_dir / "classified_methods.json"
    )
    print(f"      → {output_dir / 'classified_methods.json'}")

    ResultsExporter.export_method_sets(method_sets, output_dir / "method_sets_by_question.json")
