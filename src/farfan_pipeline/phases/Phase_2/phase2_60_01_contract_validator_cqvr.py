...
class CQVRValidator:
    ...
    def verify_method_assembly_alignment(self, contract: dict[str, Any]) -> float:
        ...
        if not orphan_sources:
            ...
            self.blockers.append(f"A2: Assembly sources not in provides: {orphan_sources[:5]}")
    ...
    def verify_validation_rules(self, contract: dict[str, Any]) -> float:
        ...
        if required_elements and required_elements.issubset(all_validation_elements):
            ...
            self.warnings.append(f"B3: Required elements not in validation rules: {missing}")
    ...
    def verify_documentation_quality(self, contract: dict[str, Any]) -> float:
        ...
        has_references = any(
            m.get("epistemological_foundation", {}).get("theoretical_framework") for m in methods
        )
        if has_references:
            score += 1.0
    ...
    def _make_triage_decision(self, score: CQVRScore) -> TriageDecision:
        ...
        if (
            score.tier1_score >= self.TIER1_PRODUCTION_THRESHOLD
            and score.total_score >= self.TOTAL_PRODUCTION_THRESHOLD
        ):
    ...
    def _generate_rationale(self, decision: TriageDecision, score: CQVRScore) -> str:
        if decision == TriageDecision.PRODUCCION:
            ...
            if score.tier1_score < self.TIER1_THRESHOLD:
                reason_parts.append(
                    f"Tier 1 score below minimum threshold ({self.TIER1_THRESHOLD})"
                )
            elif score.tier1_score < self.TIER1_PRODUCTION_THRESHOLD:
                reason_parts.append(
                    f"Tier 1 score below production threshold ({self.TIER1_PRODUCTION_THRESHOLD})"
                )
            if score.total_score < self.TOTAL_PRODUCTION_THRESHOLD:
                reason_parts.append(
                    f"Total score below production threshold ({self.TOTAL_PRODUCTION_THRESHOLD})"
                )
