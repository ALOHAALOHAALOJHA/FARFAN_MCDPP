"""
Phase 7 Performance Metrics Collector
=====================================

Comprehensive performance metrics collection for Phase 7 execution.
Tracks timing, memory usage, and resource consumption for each stage.

Architecture:
    - Thread-safe metrics collection
    - Per-stage timing and memory profiling
    - Aggregate statistics computation
    - Export to JSON for analysis

Version: 1.0.0
Author: F.A.R.F.A.N Core Architecture Team
"""
from __future__ import annotations

import json
import threading
import time
import tracemalloc
from dataclasses import dataclass, field
from datetime import datetime, timezone
from pathlib import Path
from typing import Any, Dict, List, Optional


@dataclass
class StageMetrics:
    """Metrics for a single stage execution.

    Attributes:
        stage_id: Stage identifier (e.g., "STAGE_COHERENCE_ANALYSIS")
        start_time: UTC timestamp when stage started
        end_time: UTC timestamp when stage completed
        duration_ms: Execution duration in milliseconds
        memory_mb_start: Memory usage at start in MB
        memory_mb_peak: Peak memory usage during execution in MB
        memory_mb_end: Memory usage at end in MB
        cluster_count: Number of clusters being processed
        status: Execution status (completed, failed, skipped)
        error_message: Error message if status is failed
        metadata: Additional metrics metadata
    """

    stage_id: str
    start_time: str
    end_time: str
    duration_ms: float
    memory_mb_start: float
    memory_mb_peak: float
    memory_mb_end: float
    cluster_count: int = 4
    status: str = "completed"
    error_message: Optional[str] = None
    metadata: Dict[str, Any] = field(default_factory=dict)

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary for serialization."""
        return {
            "stage_id": self.stage_id,
            "start_time": self.start_time,
            "end_time": self.end_time,
            "duration_ms": self.duration_ms,
            "memory_mb_start": self.memory_mb_start,
            "memory_mb_peak": self.memory_mb_peak,
            "memory_mb_end": self.memory_mb_end,
            "cluster_count": self.cluster_count,
            "status": self.status,
            "error_message": self.error_message,
            "metadata": self.metadata,
        }


@dataclass
class Phase7Metrics:
    """Aggregate metrics for complete Phase 7 execution.

    Attributes:
        plan_id: Unique identifier for this execution
        phase_id: Phase identifier (always "P07")
        start_time: UTC timestamp when Phase 7 started
        end_time: UTC timestamp when Phase 7 completed
        total_duration_ms: Total execution duration in milliseconds
        stage_metrics: List of individual stage metrics
        aggregate_stats: Computed aggregate statistics
        macro_score_info: Information about the generated macro score
        metadata: Additional execution metadata
    """

    plan_id: str
    phase_id: str = "P07"
    start_time: str = ""
    end_time: str = ""
    total_duration_ms: float = 0.0
    stage_metrics: List[StageMetrics] = field(default_factory=list)
    aggregate_stats: Dict[str, Any] = field(default_factory=dict)
    macro_score_info: Dict[str, Any] = field(default_factory=dict)
    metadata: Dict[str, Any] = field(default_factory=dict)

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary for serialization."""
        return {
            "plan_id": self.plan_id,
            "phase_id": self.phase_id,
            "start_time": self.start_time,
            "end_time": self.end_time,
            "total_duration_ms": self.total_duration_ms,
            "stage_metrics": [m.to_dict() for m in self.stage_metrics],
            "aggregate_stats": self.aggregate_stats,
            "macro_score_info": self.macro_score_info,
            "metadata": self.metadata,
        }

    def compute_aggregate_stats(self) -> None:
        """Compute aggregate statistics from stage metrics."""
        if not self.stage_metrics:
            self.aggregate_stats = {
                "total_stages": 0,
                "completed_stages": 0,
                "failed_stages": 0,
                "skipped_stages": 0,
                "total_duration_ms": 0.0,
                "avg_duration_ms": 0.0,
                "min_duration_ms": 0.0,
                "max_duration_ms": 0.0,
                "total_memory_mb_peak": 0.0,
                "avg_memory_mb_peak": 0.0,
                "max_memory_mb_peak": 0.0,
            }
            return

        completed = [m for m in self.stage_metrics if m.status == "completed"]
        failed = [m for m in self.stage_metrics if m.status == "failed"]
        skipped = [m for m in self.stage_metrics if m.status == "skipped"]

        durations = [m.duration_ms for m in completed]
        memory_peaks = [m.memory_mb_peak for m in self.stage_metrics]

        self.aggregate_stats = {
            "total_stages": len(self.stage_metrics),
            "completed_stages": len(completed),
            "failed_stages": len(failed),
            "skipped_stages": len(skipped),
            "total_duration_ms": sum(durations) if durations else 0.0,
            "avg_duration_ms": sum(durations) / len(durations) if durations else 0.0,
            "min_duration_ms": min(durations) if durations else 0.0,
            "max_duration_ms": max(durations) if durations else 0.0,
            "total_memory_mb_peak": sum(memory_peaks) if memory_peaks else 0.0,
            "avg_memory_mb_peak": sum(memory_peaks) / len(memory_peaks) if memory_peaks else 0.0,
            "max_memory_mb_peak": max(memory_peaks) if memory_peaks else 0.0,
        }


class Phase7MetricsCollector:
    """
    Collects performance metrics for Phase 7 execution.

    Usage:
        >>> collector = Phase7MetricsCollector(plan_id="20260122_120000")
        >>> with collector.track_stage("STAGE_COHERENCE_ANALYSIS"):
        ...     coherence = compute_coherence(cluster_scores)
        >>> metrics = collector.get_metrics()
        >>> metrics.compute_aggregate_stats()
        >>> collector.export_metrics("/path/to/metrics.json")
    """

    def __init__(self, plan_id: str, cluster_count: int = 4):
        """Initialize metrics collector.

        Args:
            plan_id: Unique execution identifier
            cluster_count: Number of clusters being processed (default: 4)
        """
        self.plan_id = plan_id
        self.cluster_count = cluster_count
        self._metrics = Phase7Metrics(plan_id=plan_id)
        self._lock = threading.Lock()
        self._current_stage: Optional[str] = None
        self._stage_start_time: Optional[float] = None
        self._stage_start_memory: Optional[float] = None
        self._enabled = True

        # Start tracing memory
        tracemalloc.start()

    def start_phase(self) -> None:
        """Mark the start of Phase 7 execution."""
        with self._lock:
            self._metrics.start_time = datetime.now(timezone.utc).isoformat()

    def end_phase(self, macro_score_info: Dict[str, Any]) -> None:
        """Mark the end of Phase 7 execution.

        Args:
            macro_score_info: Information about the generated macro score
        """
        with self._lock:
            self._metrics.end_time = datetime.now(timezone.utc).isoformat()
            self._metrics.macro_score_info = macro_score_info

            # Calculate total duration
            if self._metrics.start_time:
                start = datetime.fromisoformat(self._metrics.start_time)
                end = datetime.fromisoformat(self._metrics.end_time)
                self._metrics.total_duration_ms = (
                    end - start
                ).total_seconds() * 1000

            # Compute aggregate stats
            self._metrics.compute_aggregate_stats()

            # Stop tracing memory
            tracemalloc.stop()

    def track_stage(self, stage_id: str):
        """Context manager for tracking a single stage execution.

        Args:
            stage_id: Stage identifier (e.g., "STAGE_COHERENCE_ANALYSIS")

        Returns:
            Context manager for the stage

        Example:
            >>> with collector.track_stage("STAGE_COHERENCE_ANALYSIS"):
            ...     coherence = compute_coherence(cluster_scores)
        """
        return _StageTracker(self, stage_id)

    def record_stage_metrics(self, metrics: StageMetrics) -> None:
        """Record metrics for a completed stage.

        Args:
            metrics: StageMetrics object to record
        """
        with self._lock:
            self._metrics.stage_metrics.append(metrics)

    def get_metrics(self) -> Phase7Metrics:
        """Get the current metrics snapshot.

        Returns:
            Phase7Metrics object with current data
        """
        return self._metrics

    def export_metrics(self, output_path: Path) -> None:
        """Export metrics to JSON file.

        Args:
            output_path: Path to output JSON file
        """
        output_path = Path(output_path)
        output_path.parent.mkdir(parents=True, exist_ok=True)

        with open(output_path, 'w') as f:
            json.dump(self._metrics.to_dict(), f, indent=2, default=str)

    def get_stage_metrics(self, stage_id: str) -> Optional[StageMetrics]:
        """Get metrics for a specific stage.

        Args:
            stage_id: Stage identifier

        Returns:
            StageMetrics if found, None otherwise
        """
        for m in self._metrics.stage_metrics:
            if m.stage_id == stage_id:
                return m
        return None

    def get_slowest_stages(self, n: int = 3) -> List[StageMetrics]:
        """Get the N slowest stages by duration.

        Args:
            n: Number of stages to return

        Returns:
            List of N slowest StageMetrics objects
        """
        completed = [m for m in self._metrics.stage_metrics if m.status == "completed"]
        return sorted(completed, key=lambda m: m.duration_ms, reverse=True)[:n]

    def get_memory_intensive_stages(self, n: int = 3) -> List[StageMetrics]:
        """Get the N most memory-intensive stages by peak memory.

        Args:
            n: Number of stages to return

        Returns:
            List of N most memory-intensive StageMetrics objects
        """
        return sorted(
            self._metrics.stage_metrics,
            key=lambda m: m.memory_mb_peak,
            reverse=True
        )[:n]


class _StageTracker:
    """Context manager for tracking a single stage execution."""

    def __init__(self, collector: Phase7MetricsCollector, stage_id: str):
        """Initialize stage tracker.

        Args:
            collector: Parent metrics collector
            stage_id: Stage identifier
        """
        self.collector = collector
        self.stage_id = stage_id
        self.start_time: Optional[float] = None
        self.start_memory: Optional[float] = None
        self.peak_memory: Optional[float] = None
        self.status: str = "completed"
        self.error_message: Optional[str] = None

    def __enter__(self):
        """Start tracking stage execution."""
        if not self.collector._enabled:
            return self

        self.start_time = time.time()
        self.start_memory = tracemalloc.get_traced_memory()[0] / 1024 / 1024  # Convert to MB

        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        """End tracking stage execution and record metrics."""
        if not self.collector._enabled or self.start_time is None:
            return

        end_time = time.time()
        end_memory = tracemalloc.get_traced_memory()[1] / 1024 / 1024  # Peak in MB
        duration_ms = (end_time - self.start_time) * 1000

        # Determine status
        if exc_type is not None:
            self.status = "failed"
            self.error_message = str(exc_val)
        else:
            self.status = "completed"

        # Create metrics object
        metrics = StageMetrics(
            stage_id=self.stage_id,
            start_time=datetime.fromtimestamp(self.start_time, timezone.utc).isoformat(),
            end_time=datetime.fromtimestamp(end_time, timezone.utc).isoformat(),
            duration_ms=duration_ms,
            memory_mb_start=self.start_memory or 0.0,
            memory_mb_peak=end_memory,
            memory_mb_end=end_memory,
            cluster_count=self.collector.cluster_count,
            status=self.status,
            error_message=self.error_message,
        )

        # Record metrics
        self.collector.record_stage_metrics(metrics)

        # Don't suppress exceptions
        return False


__all__ = [
    "StageMetrics",
    "Phase7Metrics",
    "Phase7MetricsCollector",
]
