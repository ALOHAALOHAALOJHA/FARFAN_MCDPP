...
RESOURCE ENFORCEMENT (Phase Zero Stability):
...
try:
    ...
    ResourceController = None  # type: ignore[misc, assignment]
    ResourceLimits = None  # type: ignore[misc, assignment]
...
from farfan_pipeline.phases.Phase_0.primitives.providers import (
...
class EnforcedBootstrap:
    ...
    def __init__(
        ...
        Args:
            ...
            enforce: Whether to actually enforce limits (for testing)
        """
        ...
        if self.enforce and ResourceLimits is not None:
            ...
            self._limits = None
            if enforce and not ENFORCEMENT_AVAILABLE:
                logger.warning(
    ...
    def from_env(
        ...
        Args:
            ...
            calibration_profile: Calibration profile name
            abort_on_insufficient: Flag to abort on insufficient data
        ...
        if ENFORCEMENT_AVAILABLE and ResourceLimits is not None:
            limits = ResourceLimits(
                memory_mb=int(os.getenv("RESOURCE_MEMORY_MB", "2048")),
                cpu_seconds=int(os.getenv("RESOURCE_CPU_SECONDS", "300")),
