    ...
    def compute_integrity_hash(self, wiring: "WiringComponents") -> str:
        ...
        try:
            ...
            hash_data = {
                ...
                "class_registry_keys": sorted(wiring.class_registry.keys()) if wiring.class_registry else [],
            ...
            json_bytes = json.dumps(hash_data, sort_keys=True, separators=(",", ":")).encode("utf-8")
...
class WiringInitializationError(Exception):
    ...
    def __init__(self, phase: str, component: str, reason: str):
        ...
        self.reason = reason
        super().__init__(f"Wiring init failed in phase '{phase}' for component '{component}': {reason}")
