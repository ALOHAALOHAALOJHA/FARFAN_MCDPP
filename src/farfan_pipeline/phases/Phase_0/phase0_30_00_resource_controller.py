"""
Module: src.farfan_pipeline.phases.Phase_zero.phase0_30_00_resource_controller
PHASE_LABEL: Phase 0
...
class MemoryWatchdog(threading.Thread):
    ...
    def __init__(
        ...
        if not isinstance(threshold_percent, int):
            raise TypeError(
                f"threshold_percent must be int, got {type(threshold_percent).__name__}"
            )
        if not isinstance(check_interval, (int, float)):
            raise TypeError(
                f"check_interval must be int or float, got {type(check_interval).__name__}"
            )
    ...
    def _set_kernel_limits(self) -> None:
        ...
        if current_as_hard == -1 or memory_bytes < current_as_hard:
            try:
                ...
                new_hard = (
                    memory_bytes if current_as_hard == -1 else min(memory_bytes, current_as_hard)
                )
            ...
            self._limits_applied["as"] = False
            logger.debug(
                "RLIMIT_AS not lowered (current=%d, requested=%d)", current_as_hard, memory_bytes
            )
        ...
        if current_cpu_hard == -1 or self.limits.cpu_seconds < current_cpu_hard:
            try:
                new_hard = (
                    ...
                    if current_cpu_hard == -1
                    else min(self.limits.cpu_seconds, current_cpu_hard)
                )
                resource.setrlimit(
                    resource.RLIMIT_CPU,
            ...
            try:
                if limit_name == "as":
                    ...
                    restore_soft = (
                        orig_soft
                        if orig_soft == -1
                        else min(orig_soft, current_hard) if current_hard != -1 else orig_soft
                    )
