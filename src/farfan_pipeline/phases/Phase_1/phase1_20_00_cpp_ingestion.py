...
try:
    ...
    warnings.warn(
        f"Signal enrichment module not available: {e}. " "Signal-based analysis will be limited.",
        ImportWarning,
    )
...
try:
    ...
    warnings.warn(
        f"Question anchoring not available: {e}. " "Question-aware chunking will be disabled.",
        ImportWarning,
    )
...
SIGNAL_QUALITY_TIER_BOOSTS = {"EXCELLENT": 0.15, "GOOD": 0.10, "ADEQUATE": 0.05, "SPARSE": 0.0}
...
def _get_table_extractor():
    ...
    """
    try:
        from farfan_pipeline.methods.financiero_viabilidad_tablas import PDETMunicipalPlanAnalyzer
...
def _classify_table_type_lightweight(rows: list[list[str]]) -> str:
    ...
    if any(kw in header_text for kw in ["presupuesto", "recurso", "monto", "sgp", "sgr", "financ"]):
        return "presupuesto"
    if any(
        kw in header_text for kw in ["indicador", "meta", "línea base", "linea base", "producto"]
    ):
        return "indicadores"
...
def attach_tables_to_chunk(
    chunk_page_range: tuple[int, int], table_islands: list[dict]
) -> list[dict]:
    """
...
def exorcise_headers_footers(
    ...
    header_pattern = _find_common_prefix([p[:header_chars] for p in sample], min_occurrences)

    # Find common footer pattern
    footer_pattern = _find_common_suffix(
        [p[-footer_chars:] if len(p) > footer_chars else p for p in sample], min_occurrences
    )
...
def exorcise_from_text(full_text: str, pdf_path: str | None = None) -> tuple[str, dict[str, Any]]:
    """
...
def extract_document_genome(pdf_path: str, sample_pages: list[int] | None = None) -> DocumentGenome:
    """
    ...
    try:
        ...
        first_pages_text = " ".join(doc[i].get_text() for i in range(min(10, len(doc)))).lower()
...
class PADimGridSpecification:
    ...
    def validate_chunk(cls, chunk: Any) -> None:
        ...
        assert re.match(
            CHUNK_ID_PATTERN, chunk.chunk_id
        ), f"FATAL: Invalid chunk_id format {chunk.chunk_id}"

        # VALID VALUES
        assert chunk.policy_area_id in cls.POLICY_AREAS, f"FATAL: Invalid PA {chunk.policy_area_id}"
        assert chunk.dimension_id in cls.DIMENSIONS, f"FATAL: Invalid DIM {chunk.dimension_id}"
        assert 0 <= chunk.chunk_index < 60, f"FATAL: Invalid index {chunk.chunk_index}"
    ...
    def validate_chunk_set(cls, chunks: list[Any]) -> None:
        ...
        for chunk in chunks:
            assert (
                chunk.chunk_id not in seen_chunk_ids
            ), f"FATAL: Duplicate chunk_id {chunk.chunk_id}"
        ...
        assert (
            seen_chunk_ids == expected_chunk_ids
...
class Phase1FailureHandler:
    ...
    def handle_subphase_failure(sp_num: int, error: Exception) -> None:
        ...
        raise Phase1FatalError(f"Phase 1 failed at SP{sp_num}: {error}") from error
    ...
    def validate_final_state(cpp: CanonPolicyPackage) -> bool:
        ...
        validations = {
            ...
            "chunks_valid": all(
                hasattr(c, "policy_area_id") and hasattr(c, "dimension_id")
                # hasattr(c, 'strategic_rank') # Not in current Chunk model, stored in metadata or SmartChunk
                for c in chunks
            ),
            "pa_dim_complete": len(set((c.policy_area_id, c.dimension_id) for c in chunks)) == 60,
        }
...
class Phase1CPPIngestionFullContract:
    ...
    def __init__(self, signal_registry: Any | None = None):
        ...
        self.signal_enricher: Any | None = None  # Signal enrichment engine
    ...
    def _validate_canonical_input(self, canonical_input: CanonicalInput):
        ...
        # [PRE-002] document_id
        assert (
            isinstance(canonical_input.document_id, str) and len(canonical_input.document_id) > 0
        ), "FATAL [PRE-002]: document_id must be non-empty string"

        # [PRE-003] pdf_path exists
        assert (
            canonical_input.pdf_path.exists()
        ), f"FATAL [PRE-003]: pdf_path does not exist: {canonical_input.pdf_path}"

        # [PRE-004] pdf_sha256 format
        assert (
            isinstance(canonical_input.pdf_sha256, str) and len(canonical_input.pdf_sha256) == 64
        ...
        assert all(
            c in "0123456789abcdefABCDEF" for c in canonical_input.pdf_sha256
        ), "FATAL [PRE-004]: pdf_sha256 must be hexadecimal"

        # [PRE-005] questionnaire_path exists
        assert (
            canonical_input.questionnaire_path.exists()
        ), f"FATAL [PRE-005]: questionnaire_path does not exist: {canonical_input.questionnaire_path}"

        # [PRE-006] questionnaire_sha256 format
        assert (
            isinstance(canonical_input.questionnaire_sha256, str)
            and len(canonical_input.questionnaire_sha256) == 64
        ), "FATAL [PRE-006]: questionnaire_sha256 must be 64-char hex string"
        assert all(
            c in "0123456789abcdefABCDEF" for c in canonical_input.questionnaire_sha256
        ), "FATAL [PRE-006]: questionnaire_sha256 must be hexadecimal"

        # [PRE-007] validation_passed
        assert canonical_input.validation_passed is True and isinstance(
            canonical_input.validation_passed, bool
        ...
        assert (
            actual_pdf_hash == canonical_input.pdf_sha256.lower()
        ...
        assert (
            actual_q_hash == canonical_input.questionnaire_sha256.lower()
        ), f"FATAL [PRE-009]: Questionnaire integrity check failed. Expected {canonical_input.questionnaire_sha256}, got {actual_q_hash}"

        logger.info(
            "PRE-CONDITIONS VALIDATED: All 9 checks passed (PRE-010 check_dependencies skipped)"
        )
    ...
    def _assert_chunk_count(self, sp_num: int, chunks: list[Any], count: int):
        ...
        if Phase1MissionContract.is_critical(sp_num):
            logger.info(
                f"SP{sp_num} [CRITICAL WEIGHT={weight}] chunk count VALIDATED: {count} chunks"
            )
    ...
    def _validate_critical_chunk_metadata(self, chunk: SmartChunk) -> None:
        ...
        for attr_name, attr_value in required_attrs.items():
            assert attr_value is not None, f"CRITICAL: chunk {chunk.chunk_id} missing {attr_name}"
    ...
    def _assert_validation_pass(self, sp_num: int, result: ValidationResult):
        ...
        if result.status != "VALID":
            error_msg = f"SP{sp_num} [WEIGHT={weight}] validation failed: " f"{result.violations}"
            if Phase1MissionContract.is_critical(sp_num):
    ...
    def run(self, canonical_input: CanonicalInput) -> CanonPolicyPackage:
        ...
        try:
            ...
            pa_dim_chunks = self._execute_sp4_segmentation(preprocessed, structure, knowledge_graph)
            ...
            checkpoint_passed, checkpoint_errors = self.checkpoint_validator.validate_checkpoint(
                ...
                validators=[
                    ...
                    lambda x: (
                        all(isinstance(c, Chunk) for c in x),
                        "All items must be Chunk instances",
                    ),
                    lambda x: (
                        len(set(c.chunk_id for c in x)) == 60,
            ...
            integrated_causal = self._execute_sp6_causal_integration(pa_dim_chunks, causal_chains)
            ...
            smart_chunks = self._execute_sp11_smart_chunks(pa_dim_chunks, self.subphase_results)
            ...
            checkpoint_passed, checkpoint_errors = self.checkpoint_validator.validate_checkpoint(
                ...
                validators=[
                    ...
                    lambda x: (
                        all(isinstance(c, SmartChunk) for c in x),
                        "All items must be SmartChunk instances",
                    ),
                    lambda x: (
                        len(set(c.chunk_id for c in x)) == 60,
                        "All chunk_ids must be unique",
                    ...
                    lambda x: (
                        all(hasattr(c, "causal_graph") for c in x),
                        "All chunks must have causal_graph",
                    ),
                    lambda x: (
                        all(hasattr(c, "temporal_markers") for c in x),
                        "All chunks must have temporal_markers",
                    ),
                ],
            )
            if not checkpoint_passed:
            ...
            self._assert_smart_chunk_invariants(
                11, smart_chunks
            )  # HARD STOP IF FAILS - CRITICAL WEIGHT
            ...
            checkpoint_passed, checkpoint_errors = self.checkpoint_validator.validate_checkpoint(
                ...
                validators=[
                    lambda x: (
                        x.status == "VALID",
                        f"Validation status must be VALID, got {x.status}",
                    ),
                    lambda x: (x.chunk_count == 60, f"chunk_count must be 60, got {x.chunk_count}"),
                    lambda x: (
                        len(x.violations) == 0,
                        f"Must have zero violations, got {len(x.violations)}",
                    ),
                ],
            )
            if not checkpoint_passed:
    ...
    def _record_subphase(self, sp_num: int, output: Any):
        ...
        if Phase1MissionContract.is_critical(sp_num):
            ...
            logger.info(
                f"SP{sp_num} [WEIGHT={weight}] recorded: timestamp={timestamp}, hash={hash_value[:16]}..."
            )
    ...
    def _execute_sp0_language_detection(self, canonical_input: CanonicalInput) -> LanguageData:
        ...
        if LANGDETECT_AVAILABLE and len(sample_text) > 50:
            try:
                ...
                logger.info(
                    f"SP0: Detected language {primary_language} with confidence {confidence_scores.get(primary_language, 0.0):.2f}"
                )
    ...
    def _execute_sp1_preprocessing(
        self, canonical_input: CanonicalInput, lang_data: LanguageData
    ) -> PreprocessedDoc:
        """
        ...
        if PYMUPDF_AVAILABLE and canonical_input.pdf_path.exists():
            try:
                ...
                logger.info(
                    f"SP1: Extracted {len(raw_text)} characters from PDF (Total source: {total_chars})"
                )
            except Exception:
        ...
        elif canonical_input.pdf_path.exists():
            try:
        ...
        logger.info(
            f"SP1: Preprocessed {len(tokens)} tokens, {len(sentences)} sentences, {len(paragraphs)} paragraphs"
        )
    ...
    def _execute_sp2_structural(self, preprocessed: PreprocessedDoc) -> StructureData:
        ...
        if STRUCTURAL_AVAILABLE:
            try:
                ...
                raw_objects = {
                    "pages": [
                        {"text": p, "page_num": i} for i, p in enumerate(preprocessed.paragraphs)
                    ]
                }
                policy_graph = normalizer.normalize(raw_objects)
                sections = [
                    s.get("title", f"Section_{i}")
                    for i, s in enumerate(policy_graph.get("sections", []))
                ]
        ...
        if not sections:
            ...
            for i in range(len(preprocessed.paragraphs)):
                idx = min(
                    i // max(1, len(preprocessed.paragraphs) // len(sections)), len(sections) - 1
                )
        ...
        if PDT_TYPES_AVAILABLE:
            ...
            self.subphase_results["structure_annotations"] = {
                k: {
                    "nivel_jerarquico": (
                        v["nivel_jerarquico"].name if v["nivel_jerarquico"] else None
                    ),
        ...
        logger.info(
            f"SP2: Identified {len(sections)} sections, mapped {len(paragraph_mapping)} paragraphs"
        )

        return StructureData(
            sections=sections, hierarchy=hierarchy, paragraph_mapping=paragraph_mapping
        )

    def _execute_sp3_knowledge_graph(
        self, preprocessed: PreprocessedDoc, structure: StructureData
    ) -> KnowledgeGraph:
        """
        ...
        return KnowledgeGraph(nodes=nodes, edges=edges, span_to_node_mapping={})

    def _execute_sp4_segmentation(
        self, preprocessed: PreprocessedDoc, structure: StructureData, kg: KnowledgeGraph
    ) -> list[Chunk]:
        """
        ...
        for pa in PADimGridSpecification.POLICY_AREAS:
            for dim in PADimGridSpecification.DIMENSIONS:
                ...
                for para_idx, para in enumerate(preprocessed.paragraphs):
                    ...
                    signal_boost = 0
                    if (
                        self.signal_enricher is not None
                        and pa in self.signal_enricher.context.signal_packs
                    ):
        ...
        ), f"SP4 FATAL: Coverage mismatch. Missing: {expected_ids - chunk_ids}"

        logger.info("SP4: Generated EXACTLY 60 chunks with complete PA×DIM coverage")
        return chunks
    ...
    def _execute_sp5_causal_extraction(self, chunks: list[Chunk]) -> CausalChains:
        ...
        for chunk in chunks:
            chunk_text = (
                chunk.segmentation_metadata.get("text", "")
                if hasattr(chunk, "segmentation_metadata")
                else ""
            )
                ...
                if keyword.lower() in chunk_text.lower():
                    ...
                    for match in matches[:3]:  # Limit to 3 per keyword
                        ...
                        if BEACH_CLASSIFY is not None:
                            ...
                            sufficiency = (
                                0.7 if keyword in ["garantiza", "asegura", "produce"] else 0.4
                            )
            ...
            chunk.causal_graph = CausalGraph(
                events=events[:10], causes=causes[:5], effects=effects[:5]
            )

            if events:
                causal_chains_list.append(
                    {"chunk_id": chunk.chunk_id, "chain_count": len(events), "events": events[:5]}
                )

        logger.info(
            f"SP5: Extracted causal chains from {len(causal_chains_list)} chunks (Beach={DEREK_BEACH_AVAILABLE})"
        )

        return CausalChains(chains=causal_chains_list)

    def _execute_sp6_causal_integration(
        self, chunks: list[Chunk], chains: CausalChains
    ) -> IntegratedCausal:
        """
        ...
        chunk_texts = {
            c.chunk_id: c.segmentation_metadata.get("text", "")[:500].lower()
            for c in chunks
            if hasattr(c, "segmentation_metadata")
        }
        ...
        if TEORIA_CAMBIO_AVAILABLE and TEORIA_CAMBIO_CLASS is not None and cross_chunk_links:
            try:
                ...
                for link in cross_chunk_links[:20]:  # Limit for performance
                    ...
                    source_cat = (
                        "producto"
                        if "DIM03" in source_dim
                        else ("insumo" if "DIM01" in source_dim else "resultado")
                    )
                    target_cat = (
                        "resultado"
                        if "DIM04" in target_dim
                        else ("producto" if "DIM03" in target_dim else "causalidad")
                    )
                ...
                teoria_cambio_metadata = {
                    ...
                    "violaciones_orden": (
                        len(validation_result.violaciones_orden) if validation_result else 0
                    ),
                    "caminos_completos": (
                        len(validation_result.caminos_completos) if validation_result else 0
                    ),
                }
                logger.info(
                    f"SP6: TeoriaCambio validation: es_valida={validation_result.es_valida if validation_result else 'N/A'}"
                )
            except Exception as e:
        ...
        logger.info(
            f"SP6: Integrated {len(global_events)} events, {len(cross_chunk_links)} cross-chunk links (TeoriaCambio={TEORIA_CAMBIO_AVAILABLE})"
        )
    ...
    def _execute_sp7_arguments(
        self, chunks: list[Chunk], integrated: IntegratedCausal
    ) -> Arguments:
        """
                ...
                for pattern in patterns:
                    ...
                    for match in matches[:2]:
                        ...
                        if self.signal_enricher is not None:
                            ...
                            arg_entry["supporting_signals"] = signal_score.get(
                                "supporting_signals", []
                            )

                        chunk_arguments[
                            arg_type + "s" if not arg_type.endswith("s") else arg_type
                        ].append(arg_entry)
            ...
            if BEACH_CLASSIFY is not None:
                ...
                if self.signal_enricher is not None:
                    ...
                    evidence_signal_scores = [
                        ev.get("signal_score", 0.0)
                        for ev in chunk_arguments["evidence"]
                        if ev.get("signal_score") is not None
                    ]
                    if evidence_signal_scores:
                        signal_boost = (
                            sum(evidence_signal_scores)
                            / len(evidence_signal_scores)
                ...
                sufficiency = min(
                    ...
                    + (claim_count * 0.1)
                    + (evidence_count * 0.1)
                    + signal_boost * SIGNAL_BOOST_SUFFICIENCY_COEFFICIENT,
                )
        ...
        logger.info(
            f"SP7: Analyzed arguments for {len(arguments_map)} chunks (Beach={DEREK_BEACH_AVAILABLE})"
        )
    ...
    def _execute_sp8_temporal(self, chunks: list[Chunk], integrated: IntegratedCausal) -> Temporal:
        ...
        for chunk in chunks:
            ...
            if self.signal_enricher is not None:
                signal_temporal_markers = (
                    self.signal_enricher.extract_temporal_markers_with_signals(chunk_text, pa_id)
                )
                ...
                for marker in signal_temporal_markers:
                    if marker["type"] == "YEAR":
                        try:
                            ...
                            logging.debug(
                                f"Failed to extract year from marker text: {marker['text']!r}"
                            )
            ...
            if temporal_markers["verb_sequence"]:
                temporal_markers["temporal_order"] = min(
                    v[1] for v in temporal_markers["verb_sequence"]
                )
        ...
        logger.info(
            f"SP8: Extracted temporal markers from {len(timeline)} chunks with temporal content"
        )
    ...
    def _execute_sp9_discourse(self, chunks: list[Chunk], arguments: Arguments) -> Discourse:
        ...
        for chunk in chunks:
            ...
            if (
                self.signal_enricher is not None
                and pa_id in self.signal_enricher.context.signal_packs
            ):
                ...
                for pattern in signal_pack.patterns[:MAX_SIGNAL_PATTERNS_DISCOURSE]:
                    ...
                    try:
                        if re.search(pattern, chunk_lower, re.IGNORECASE):
                            ...
                            if any(
                                kw in pattern_lower
                                for kw in ["debe", "deberá", "requiere", "obligator"]
                            ):
                                mode_scores["injunctive"] = (
                                    mode_scores.get("injunctive", 0)
                                    + DISCOURSE_SIGNAL_BOOST_INJUNCTIVE
                                )
                            elif any(
                                kw in pattern_lower for kw in ["por tanto", "debido", "porque"]
                            ):
                                mode_scores["argumentative"] = (
                                    mode_scores.get("argumentative", 0)
                                    + DISCOURSE_SIGNAL_BOOST_ARGUMENTATIVE
                                )
                            elif any(
                                kw in pattern_lower for kw in ["define", "consiste", "significa"]
                            ):
                                mode_scores["expository"] = (
                                    mode_scores.get("expository", 0)
            ...
            dominant_mode = (
                max(mode_scores.keys(), key=lambda k: mode_scores[k])
    ...
    def _execute_sp10_strategic(
        ...
        for chunk in chunks:
            ...
            temporal_order = (
                chunk.temporal_markers.get("temporal_order", 5) if chunk.temporal_markers else 5
            )
            ...
            if self.signal_enricher is not None and isinstance(arg_data, dict):
                ...
                for ev in arg_data.get("evidence", []):
                    if isinstance(ev, dict) and ev.get("signal_score") is not None:
                        signal_boost += (
                            ev["signal_score"] * 0.1
                        )  # Boost from signal-enhanced evidence
            ...
            if self.signal_enricher is not None:
                ...
                if pa_id in self.signal_enricher.context.quality_metrics:
                    ...
                    signal_quality_boost = SIGNAL_QUALITY_TIER_BOOSTS.get(
                        metrics.coverage_tier, 0.0
                    )
    ...
    def _execute_sp11_smart_chunks(
        self, chunks: list[Chunk], enrichments: dict[int, Any]
    ) -> list[SmartChunk]:
        """
        ...
        for idx, chunk in enumerate(chunks):
            try:
                ...
                kwargs = {
                    ...
                    "strategic_rank": (
                        chunk.strategic_rank if hasattr(chunk, "strategic_rank") else 0
                    ),
                    "irrigation_links": [],
        ...
        if len(smart_chunks) != 60:
            raise Phase1FatalError(
                f"SP11 FATAL: Generated {len(smart_chunks)} SmartChunks, MUST be EXACTLY 60"
            )
        ...
        logger.info("SP11: Generated EXACTLY 60 SmartChunks with complete PA×DIM coverage")
        ...
        for chunk in chunks:
            ...
            for other in chunk_by_pa.get(chunk.policy_area_id, []):
                if other.chunk_id != chunk.chunk_id:
                    links.append(
                        {"target": other.chunk_id, "type": "same_policy_area", "strength": 0.7}
                    )
            ...
            for other in chunk_by_dim.get(chunk.dimension_id, []):
                if other.chunk_id != chunk.chunk_id:
                    links.append(
                        {"target": other.chunk_id, "type": "same_dimension", "strength": 0.6}
                    )
            ...
            if chunk.causal_graph and chunk.causal_graph.effects:
                for other in chunks:
                    if (
                        ...
                        and other.causal_graph
                        and other.causal_graph.causes
                    ):
                        # Check for overlap in effects -> causes
                        chunk_effects = set(
                            str(e).lower()[:50] for e in chunk.causal_graph.effects if e
                        )
                        other_causes = set(
                            str(c).lower()[:50] for c in other.causal_graph.causes if c
                        )

                        if chunk_effects & other_causes:  # Intersection
                            links.append(
                                {"target": other.chunk_id, "type": "causal_flow", "strength": 0.9}
                            )
                                ...
                                links.append(
                                    {
                                        ...
                                        "shared_signals": list(
                                            chunk_signal_tags & other_signal_tags
                                        )[:MAX_SHARED_SIGNALS_DISPLAY],
                                    }
                                )
                ...
                if chunk.signal_scores:
                    for other in chunks:
                        if other.chunk_id != chunk.chunk_id and other.signal_scores:
                            ...
                            common_signal_types = set(chunk.signal_scores.keys()) & set(
                                other.signal_scores.keys()
                            )
    ...
    def _execute_sp14_deduplication(self, chunks: list[SmartChunk]) -> list[SmartChunk]:
        ...
        if len(unique_chunks) != 60:
            raise Phase1FatalError(
                f"SP14 FATAL: Output has {len(unique_chunks)} chunks, MUST be EXACTLY 60"
            )
        ...
        if chunk_ids != expected_ids:
            raise Phase1FatalError("SP14 FATAL: Coverage lost during deduplication")
    ...
    def _execute_sp15_ranking(self, chunks: list[SmartChunk]) -> list[SmartChunk]:
        ...
        ranked_chunks = sorted(
            chunks, key=lambda c: priorities.get(c.chunk_id, {}).get("rank", 0), reverse=True
        )
        ...
        for chunk_id, ranking in final_rankings.items():
            ...
            if not (0 <= rank <= 100):
                raise Phase1FatalError(
                    f"SP15 FATAL: strategic_rank {rank} for {chunk_id} out of range [0, 100]"
                )
    ...
    def _construct_cpp_with_verification(self, ranked: list[SmartChunk]) -> CanonPolicyPackage:
        ...
        if len(chunk_graph.chunks) != 60:
            raise Phase1FatalError(
                f"CPP FATAL: ChunkGraph has {len(chunk_graph.chunks)} chunks, MUST be 60"
            )
        ...
        if SISAS_AVAILABLE and SignalPack is not None:
            ...
            try:
                    ...
                    if self.signal_registry is not None:
                        ...
                        try:
                            ...
                            if pack is None:
                                ...
                                logger.warning(
                                    f"Signal registry missing PA {pa_id}, using default pack"
                                )
                                pack = create_default_signal_pack(pa_id)
                        except Exception as e:
                            logger.warning(
                                f"Error getting signal pack for {pa_id}: {e}, using default"
                            )
                            pack = create_default_signal_pack(pa_id)
                    else:
                        # DEGRADED MODE: No registry injected (should not happen in production)
                        logger.warning(
                            "Phase 1 running without signal_registry (policy violation), using default packs"
                        )
                        pack = create_default_signal_pack(pa_id)
                ...
                quality_metrics = QualityMetrics.compute_from_sisas(
                    signal_packs=signal_packs, chunks=chunk_graph.chunks
                )
                logger.info(
                    f"CPP: Computed QualityMetrics from SISAS - provenance={quality_metrics.provenance_completeness:.2f}, structural={quality_metrics.structural_consistency:.2f}"
                )
            except Exception as e:
                logger.warning(
                    f"CPP: SISAS quality calculation failed: {e}, using validated defaults"
                )
        ...
        logger.info(
            f"CPP: Computed IntegrityIndex - blake2b_root={integrity_index.blake2b_root[:32]}..."
        )
        ...
        if self.signal_enricher is not None:
            try:
                signal_coverage_metrics = self.signal_enricher.compute_signal_coverage_metrics(
                    ranked
                )
        ...
        metadata = {
            ...
            "run_id": str(hash(datetime.now(UTC).isoformat())),
        ...
        chunks_with_enums = sum(
            ...
            if hasattr(c, "policy_area")
            and c.policy_area is not None
            and hasattr(c, "dimension")
            and c.dimension is not None
        )
        type_coverage_pct = (chunks_with_enums / 60) * 100 if chunks_with_enums else 0

        logger.info(
            f"CPP Construction: Built VALIDATED CanonPolicyPackage with {len(chunk_graph.chunks)} chunks"
        )
        logger.info(
            f"CPP Type Enums: {chunks_with_enums}/60 chunks ({type_coverage_pct:.1f}%) have PolicyArea/DimensionCausal enums for value aggregation"
        )
    ...
    def _verify_all_postconditions(self, cpp: CanonPolicyPackage):
        ...
        if cpp.schema_version != "CPP-2025.1":
            raise Phase1FatalError(
                f"POST FATAL: schema_version={cpp.schema_version}, MUST be 'CPP-2025.1'"
            )
        ...
        if len(trace) != 16:
            raise Phase1FatalError(
                f"POST FATAL: execution_trace has {len(trace)} entries, MUST be 16"
            )
        ...
        if actual_labels != expected_labels:
            raise Phase1FatalError(
                f"POST FATAL: execution_trace labels {actual_labels} != expected {expected_labels}"
            )
        ...
        if len(chunk_ids) != expected_count:
            raise Phase1FatalError(
                f"POST FATAL: Unique chunk_ids={len(chunk_ids)}, MUST be {expected_count}"
            )
        ...
        if not weight_metrics:
            logger.warning(
                "Weight metrics missing from metadata - contract compliance cannot be fully verified"
            )
        else:
            ...
            logger.info("  ✓ Weight contract compliance verified")
        ...
        logger.info("  ✓ chunk_count = 60")
        logger.info("  ✓ schema_version = CPP-2025.1")
        logger.info("  ✓ execution_trace = 16 entries (SP0-SP15)")
        logger.info("  ✓ PA×DIM coverage = COMPLETE")
        logger.info("  ✓ Weight-based contract compliance = VERIFIED")
    ...
    def _infer_nivel_jerarquico(self, section_text: str) -> HierarchyLevel | None:
    ...
    def _infer_seccion_pdt(self, section_text: str, snippet: str) -> PDTSectionType | None:
...
def execute_phase_1_with_full_contract(
    canonical_input: CanonicalInput, signal_registry: Any | None = None
) -> CanonPolicyPackage:
    """
    ...
    try:
        ...
        print("  - Circuit breaker: CLOSED (all systems operational)")
        return cpp
