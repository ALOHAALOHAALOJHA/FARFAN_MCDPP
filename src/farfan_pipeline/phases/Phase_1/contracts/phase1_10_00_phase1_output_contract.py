...
PHASE1_OUTPUT_POSTCONDITIONS: list[Phase1OutputPostcondition] = [
    ...
    Phase1OutputPostcondition(
        "POST-06", "Schema version must match CPP-2025.1", "validate_schema_version", "STANDARD"
    ),
]
...
def validate_phase1_output_contract(cpp: Any) -> bool:
    ...
    from farfan_pipeline.phases.Phase_1.phase1_models import SmartChunk
    ...
    for chunk in cpp.chunk_graph.chunks:
        if chunk.chunk_id not in visited:
            if has_cycle(chunk.chunk_id):
                raise ValueError("POST-03 failed: Chunk graph contains cycle")

    # POST-04: Execution trace
    if len(cpp.metadata.execution_trace) != 16:
        raise ValueError(
            f"POST-04 failed: Expected 16 execution trace entries, got {len(cpp.metadata.execution_trace)}"
        )

    # POST-05: Quality metrics
    if not cpp.quality_metrics:
        raise ValueError("POST-05 failed: Quality metrics missing")

    # POST-06: Schema version
    if cpp.metadata.schema_version != "CPP-2025.1":
        raise ValueError(
            f"POST-06 failed: Expected schema version CPP-2025.1, got {cpp.metadata.schema_version}"
        )
