    ...
    for (area_id, dim_id), group_results in grouped.items():
        try:
            ...
            dim_score = aggregator.aggregate_dimension(
                group_results, group_by_values={"policy_area": area_id, "dimension": dim_id}
            )
...
async def aggregate_policy_areas_async(
    ...
    for (area_id,), group_dims in grouped.items():
        try:
            ...
            area_score = aggregator.aggregate_area(group_dims, group_by_values={"area_id": area_id})
...
def aggregate_clusters(
    ...
    clusters = questionnaire.get("blocks", {}).get("niveles_abstraccion", {}).get("clusters", [])
    ...
    for cluster_def in clusters:
        ...
        logger.debug(f"Processing cluster {cluster_id} with expected areas: {expected_areas}")

        # Filter areas for this cluster
        cluster_areas = [area for area in policy_area_scores if area.area_id in expected_areas]
        ...
        try:
            logger.debug(f"Aggregating cluster {cluster_id} with {len(cluster_areas)} areas")

            cluster_score = aggregator.aggregate_cluster(
                cluster_areas, group_by_values={"cluster_id": cluster_id}
            )
