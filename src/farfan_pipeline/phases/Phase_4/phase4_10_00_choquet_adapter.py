...
class FuzzyMeasureGenerator:
    ...
    def _get_all_subsets(self, max_size: int | None = None) -> list[frozenset]:
        ...
        for subset in self._get_all_subsets():
            ...
            for r in range(n_minus_a + 1):
                for combo in combinations(all_remaining, r):
                    ...
                    # Formula: sum_{K ⊆ N \ A} [|K|!  (n-|K|-|A|)! / (n-|A|+1)!] m(A ∪ K)
...
class ChoquetAggregator: 
    ...
    def aggregate(
        ...
        # If we want "contribution to the score", usually we use Shapley(v) * x_i
        # if linear approximation, or interaction decomposition. 
        ...
        # For a single point estimate, we don't have a distribution unless
        # we assume input uncertainty.  Assuming inputs are exact for this calculation. 
...
class ChoquetProcessingAdapter:
    ...
    def _project_to_k_additive(self, config: ChoquetConfig, k:  int) -> ChoquetConfig:
        ...
        new_mobius = {
            subset: val
            for subset, val in config.mobius_transform.items()
            if len(subset) <= k
        }
