...
class BootstrapConvergenceAnalyzer:
    ...
    def _calculate_autocorrelation(self, samples: Sequence[float], max_lag: int = 100) -> List[float]:
        ...
        if var < 1e-12: return [1.0]*max_lag
    ...
    def _calculate_bimodality(self, samples: Sequence[float]) -> float:
        ...
        v1, v2 = statistics.variance(g1) if len(g1)>1 else 0, statistics.variance(g2) if len(g2)>1 else 0
        denom = 2 * math.sqrt((v1 + v2)/2) # Pooled variance approx
        return abs(m1 - m2) / denom if denom > 1e-9 else 0.0

    def analyze_convergence(self, samples: Sequence[float], metrics: UncertaintyMetrics = None) -> ConvergenceDiagnostics:
        n = len(samples)
        ...
        pathologies = []
        if n < 1000: pathologies.append("WARNING: Low sample count")
        if metrics and metrics.std_error < 1e-12: pathologies.append("CRITICAL: Zero variance")
        if bimod > 2.0: pathologies.append("CRITICAL: Bimodality detected")
...
class BootstrapAggregator:
    ...
    def compute_bca_interval(self, data: Sequence[float], func: Callable, alpha: float = 0.05) -> UncertaintyMetrics:
        theta_hat = func(data)
        n = len(data)
        boots = []
    ...
    def compute_with_convergence(self, data: Sequence[float], func: Callable, max_iterations: int = 10000) -> Tuple[UncertaintyMetrics, ConvergenceDiagnostics]:
        ...
        boots = [func([self._rng.choice(data) for _ in range(len(data))]) for _ in range(self.iterations)]
