...
def wilson_score_interval(p_hat: float, n: int, alpha: float = 0.05) -> tuple[float, float]:
    """
...
def weighted_aggregation(scores: list[float], weights: list[float], validate: bool = True) -> float:
    """
    ...
    if validate:
        ...
        if len(scores) != len(weights):
            raise ValueError(
                f"Scores and weights must have same length: " f"{len(scores)} vs {len(weights)}"
            )
...
def dempster_combination(
    m1_focal: dict[frozenset[str], float], m2_focal: dict[frozenset[str], float]
    ...
    normalized = {focal: mass / normalization for focal, mass in combined.items()}
...
def calibrate_confidence(
    estimated_confidence: float, n_observations: int, target_coverage: float = 0.95
) -> float:
    """
...
def validate_scoring_invariants(
    score: float, quality_threshold: float, confidence_interval: tuple[float, float]
) -> dict[str, bool]:
    """
...
def verify_convexity_property(scores: list[float], weights: list[float]) -> bool:
    """
...
def verify_wilson_monotonicity(p_hat1: float, p_hat2: float, n: int) -> bool:
    """
