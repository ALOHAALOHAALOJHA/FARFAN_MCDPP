...
class Phase0ValidationResult:
    ...
    P1_HARDENING_GATES: tuple[str, ...] = (
        "questionnaire_integrity",
        "method_registry",
        "smoke_tests",
    )
...
class Orchestrator:
    ...
    def __init__(
        ...
        if phase0_validation is not None:
            if not phase0_validation.all_passed:
                ...
                mandatory_failed = [
                    g for g in failed if g.gate_name in Phase0ValidationResult.MANDATORY_GATES
                ]
                p1_hardening_failed = [
                    g for g in failed if g.gate_name in Phase0ValidationResult.P1_HARDENING_GATES
                ]
            ...
            logger.info(
                ...
                questionnaire_sha256=(
                    phase0_validation.questionnaire_sha256[:16] + "..."
                    if phase0_validation.questionnaire_sha256
                    else "N/A"
                ),
    ...
    def _load_configuration(self) -> dict[str, Any]:
        ...
        if self.phase0_validation is not None:
            ...
            if not self.phase0_validation.has_mandatory_gates_passed():
                ...
                mandatory_failed = [
                    g for g in failed_gates if g.gate_name in Phase0ValidationResult.MANDATORY_GATES
                ]
                raise RuntimeError(
            ...
            if not self.phase0_validation.has_p1_hardening_gates_passed():
                ...
                if is_prod:
                    ...
                    p1_failed = [
                        ...
                        if g.gate_name in Phase0ValidationResult.P1_HARDENING_GATES
                    ]
                    raise RuntimeError(
                    ...
                    phase0_alignment["warnings"].append(
                        "P1 Hardening gates not fully passed (non-PROD mode)"
                    )
            else:
                phase0_alignment["gates_validated"].extend(
                    list(Phase0ValidationResult.P1_HARDENING_GATES)
                )
        else:
    ...
    async def _generate_recommendations(
        ...
        """FASE 8: Generate recommendations using SOTA recommendations engine.

        Uses multi-criteria decision analysis (TOPSIS), causal inference,
        and explainable AI for generating actionable policy recommendations.
        """
        self._ensure_not_aborted()
        ...
        try:
            # Import SOTA recommendations engine
            from farfan_pipeline.orchestration.phase8_sota_recommendations_engine import (
                generate_sota_recommendations,
            )

            # Extract cluster and area scores from macro result
            cluster_scores = {}
            area_scores = {}

            for cluster_result in macro_result.cluster_results:
                cluster_id = cluster_result.cluster_id
                cluster_scores[cluster_id] = cluster_result.cluster_score

                for area_result in cluster_result.area_scores:
                    area_id = area_result.area_id
                    area_scores[area_id] = area_result.area_score

            # Get budget from config
            budget = config.get("phase8_budget", 1000.0)

            # Generate recommendations using SOTA engine
            recommendations = generate_sota_recommendations(
                macro_score=macro_result.macro_score,
                cluster_scores=cluster_scores,
                area_scores=area_scores,
                budget=budget,
                time_horizon_months=config.get("phase8_time_horizon", 12),
                n_bootstrap_samples=config.get("phase8_bootstrap_samples", 1000),
                random_seed=config.get("random_seed"),
            )

            logger.info(
                "Phase 8 SOTA recommendations generated",
                n_recommendations=len(recommendations.get("recommendations", [])),
                total_impact=recommendations.get("total_expected_impact", 0),
                budget_utilization=f"{recommendations.get('budget_utilization', 0):.1%}",
            )

            instrumentation.increment(items_done=1)
            return recommendations

        except Exception as e:
            logger.error(f"Phase 8 SOTA recommendations failed: {e}", exc_info=True)
            # Fallback to stub on error
            logger.warning("Falling back to stub recommendations due to error")
            recommendations = {
                "status": "fallback_stub",
                "macro_score": macro_result.macro_score,
                "error": str(e),
            }
            instrumentation.record_error("recommendations", str(e))
            return recommendations
