...
def _normalize_monolith_for_hash(
    monolith: dict[str, Any] | MappingProxyType[str, Any]
) -> dict[str, Any]:
...
class Evidence:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.modality, str):
            raise TypeError(f"modality must be str, got {type(self.modality)}")
        if not isinstance(self.elements, list):
            raise TypeError(f"elements must be list, got {type(self.elements)}")
        if not isinstance(self.raw_results, dict):
            raise TypeError(f"raw_results must be dict, got {type(self.raw_results)}")
...
class PhaseResult:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.success, bool):
            raise TypeError(f"success must be bool, got {type(self.success)}")
        if not isinstance(self.phase_id, str):
            raise TypeError(f"phase_id must be str, got {type(self.phase_id)}")
        if self.error is not None and not isinstance(self.error, Exception):
            raise TypeError(f"error must be Exception or None, got {type(self.error)}")
        if not isinstance(self.duration_ms, (int, float)):
            raise TypeError(f"duration_ms must be float, got {type(self.duration_ms)}")
        if not isinstance(self.mode, str):
            raise TypeError(f"mode must be str, got {type(self.mode)}")
        if not isinstance(self.aborted, bool):
            raise TypeError(f"aborted must be bool, got {type(self.aborted)}")
...
class MicroQuestionRun:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.question_id, str):
            raise TypeError(f"question_id must be str, got {type(self.question_id)}")
        if not isinstance(self.question_global, int):
            raise TypeError(
                f"question_global must be int, got {type(self.question_global)}"
            )
        if not isinstance(self.base_slot, str):
            raise TypeError(f"base_slot must be str, got {type(self.base_slot)}")
        ...
        if self.evidence is not None and not isinstance(self.evidence, Evidence):
            raise TypeError(
                f"evidence must be Evidence or None, got {type(self.evidence)}"
            )
        if self.error is not None and not isinstance(self.error, str):
            raise TypeError(f"error must be str or None, got {type(self.error)}")
        if self.duration_ms is not None and not isinstance(
            self.duration_ms, (int, float)
        ):
            raise TypeError(
                f"duration_ms must be float or None, got {type(self.duration_ms)}"
            )
        if not isinstance(self.aborted, bool):
            raise TypeError(f"aborted must be bool, got {type(self.aborted)}")
...
class ScoredMicroQuestion:
    ...
    def __post_init__(self) -> None:
        ...
        if self.score is not None and not isinstance(self.score, (int, float)):
            raise TypeError(f"score must be float or None, got {type(self.score)}")
        if self.normalized_score is not None and not isinstance(
            self.normalized_score, (int, float)
        ):
            raise TypeError(
                f"normalized_score must be float or None, got {type(self.normalized_score)}"
            )
        if self.quality_level is not None and not isinstance(self.quality_level, str):
            raise TypeError(
                f"quality_level must be str or None, got {type(self.quality_level)}"
            )
        if self.evidence is not None and not isinstance(self.evidence, Evidence):
            raise TypeError(
                f"evidence must be Evidence or None, got {type(self.evidence)}"
            )
        if not isinstance(self.scoring_details, dict):
            raise TypeError(
                f"scoring_details must be dict, got {type(self.scoring_details)}"
            )
        if not isinstance(self.metadata, dict):
            raise TypeError(f"metadata must be dict, got {type(self.metadata)}")
        if self.error is not None and not isinstance(self.error, str):
...
async def execute_phase_with_timeout(
    ...
    try:
        result = await asyncio.wait_for(
            target(*call_args, **call_kwargs), timeout=timeout_s
        )
        ...
        elapsed = time.perf_counter() - start
        logger.error(
            f"Fase {phase_id} ERROR: {exc} (después de {elapsed:.2f}s)", exc_info=True
        )
        raise
...
class MethodExecutor:
    ...
    def inject_method(
        self, class_name: str, method_name: str, method: Callable[..., Any]
    ) -> None:
...
def validate_phase_definitions(
    ...
    if phase_ids[-1] != len(phase_list) - 1:
        raise RuntimeError(
            f"Phase IDs must be contiguous from 0 to {len(phase_list) - 1}. Got highest ID: {phase_ids[-1]}"
        )
    ...
    for phase_id, mode, handler_name, label in phase_list:
        if mode not in valid_modes:
            raise RuntimeError(
                f"Phase {phase_id} ({label}): invalid mode '{mode}'. Mode must be one of {valid_modes}"
            )

        if not hasattr(orchestrator_class, handler_name):
            raise RuntimeError(
                f"Phase {phase_id} ({label}): handler method '{handler_name}' does not exist in {orchestrator_class.__name__}"
            )

        handler = getattr(orchestrator_class, handler_name, None)
        if not callable(handler):
            raise RuntimeError(
                f"Phase {phase_id} ({label}): handler '{handler_name}' is not callable"
            )
...
class Orchestrator:
    ...
    def __init__(
        ...
        if processor_bundle is not None:
            if hasattr(processor_bundle, "enriched_signal_packs"):
                ...
                logger.info(
                    f"Orchestrator wired with {len(self._enriched_packs) if self._enriched_packs else 0} enriched signal packs"
                )
            else:
                logger.warning(
                    "ProcessorBundle provided but missing enriched_signal_packs attribute"
                )
        else:
            logger.warning(
                "No ProcessorBundle provided - signal enrichment unavailable"
            )
        ...
        # ========================================================================
        if (
            not hasattr(self.executor, "signal_registry")
            or self.executor.signal_registry is None
        ):
        ...
        if self.recommendation_engine is not None:
            ...
            logger.warning(
                "No RecommendationEngine port provided - recommendations unavailable"
            )
    ...
    def process_development_plan(
        ...
        if loop and loop.is_running():
            raise RuntimeError(
                "process_development_plan() must be called outside an active asyncio loop"
            )
