...
MAX_COMPATIBILITY_THRESHOLD = 0.99
...
class CalibrationValidator:
    ...
    def validate_layer_completeness(
        self, method_id: str, role: MethodRole, declared_layers: set[LayerType]
    ) -> tuple[bool, list[str]]:
        """
        ...
        errors = [
            ...
            f"{[layer.value for layer in missing]}"
        ]
        return False, errors

    def validate_fusion_weights(
        self, role_params: dict[str, Any], role_name: str
    ) -> tuple[bool, list[str]]:
        """
    ...
    def validate_anti_universality(
        ...
        for _policy_id, policy_spec in policy_areas.items():
            if policy_spec.get("default_score", 0.0) < MAX_COMPATIBILITY_THRESHOLD:
    ...
    def validate_intrinsic_calibration(
        ...
        weight_sum = (
            weights.get("w_th", 0) + weights.get("w_imp", 0) + weights.get("w_dep", 0)
        )
    ...
    def validate_boundedness(
        self, layer_scores: dict[str, float], calibrated_score: float
    ) -> tuple[bool, list[str]]:
        """
...
def validate_config_files(config_dir: str | None = None) -> tuple[bool, list[str]]:
    """
...
def validate_certificate(
    ...
    valid, errors = validator.validate_boundedness(
        certificate.layer_scores, certificate.calibrated_score
    )
