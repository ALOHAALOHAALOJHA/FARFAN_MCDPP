...
class CompatibilityRegistry:
    ...
    def _load(self):
        ...
        if "method_compatibility" not in data:
            raise ValueError("Config must have 'method_compatibility' key at top level")
        ...
        logger.info(
            "compatibility_registry_loaded", extra={"total_methods": len(self.mappings)}
        )
    ...
    def get(self, method_id: str) -> CompatibilityMapping:
        ...
        if method_id not in self.mappings:
            logger.warning(
                ...
                extra={"method": method_id, "default_score": 0.1},
            )
            ...
            if not is_compliant:
                ...
                logger.error(
                    ...
                    extra={
                        "method": method_id,
                        "avg_q": (
                            sum(mapping.questions.values()) / len(mapping.questions)
                            if mapping.questions
                            else 0
                        ),
                        "avg_d": (
                            sum(mapping.dimensions.values()) / len(mapping.dimensions)
                            if mapping.dimensions
                            else 0
                        ),
                        "avg_p": (
                            sum(mapping.policies.values()) / len(mapping.policies)
                            if mapping.policies
                            else 0
                        ),
                    },
...
class ContextualLayerEvaluator:
    ...
    def evaluate_question(self, method_id: str, question_id: str) -> float:
        ...
        logger.debug(
            ...
            extra={"method": method_id, "question": question_id, "score": score},
        )
    ...
    def evaluate_dimension(self, method_id: str, dimension: str) -> float:
        ...
        logger.debug(
            ...
            extra={"method": method_id, "dimension": dimension, "score": score},
        )
    ...
    def evaluate_policy(self, method_id: str, policy_area: str) -> float:
        ...
        logger.debug(
            ...
            extra={"method": method_id, "policy": policy_area, "score": score},
        )

        return score

    def evaluate_all_contextual(
        self, method_id: str, question_id: str, dimension: str, policy_area: str
    ) -> dict[str, float]:
        """
