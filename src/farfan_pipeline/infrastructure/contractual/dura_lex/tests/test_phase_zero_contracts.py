...
from canonic_phases.phase_0_input_validation.phase0_boot_checks import BootCheckError
...
from canonic_phases.phase_0_input_validation.phase0_verified_pipeline_runner import VerifiedPipelineRunner
...
def test_contract_08_prod_mode_must_fail_on_boot_check_error():
    ...
    """
    from canonic_phases.phase_0_input_validation.phase0_boot_checks import run_boot_checks
    
    config = MagicMock()
    config.mode.value = "prod"
    
    # Mock a failing dependency check
    with patch('canonic_phases.Phase_zero.boot_checks._check_calibration_files') as mock_check:
...
def test_contract_09_dev_mode_must_warn_on_boot_check_failure():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch('canonic_phases.Phase_zero.verified_pipeline_runner.RuntimeConfig') as mock_config:
            ...
            with patch('canonic_phases.Phase_zero.verified_pipeline_runner.run_boot_checks') as mock_boot:
