...
class BaseAggregationContract(ABC):
    ...
    def validate_weight_normalization(
        self, weights: list[float], context: dict[str, Any] | None = None
    ) -> bool:
        """
        ...
        if abs(weight_sum - expected) > self.WEIGHT_TOLERANCE:
            ...
            self.violations.append(violation)
            logger.error(
                f"[{self.contract_id}] Weight normalization violation: {violation.message}"
            )
            if self.abort_on_violation:
                raise ValueError(f"[{self.contract_id}] {violation.message}")
            return False
    ...
    def validate_score_bounds(self, score: float, context: dict[str, Any] | None = None) -> bool:
        """
    ...
    def validate_coherence_bounds(
        self, coherence: float, context: dict[str, Any] | None = None
    ) -> bool:
        """
    ...
    def validate_convexity(
        self, aggregated: float, inputs: list[float], context: dict[str, Any] | None = None
    ) -> bool:
        """
    ...
    def validate_hermeticity(
        self, actual_ids: set[str], expected_ids: set[str], context: dict[str, Any] | None = None
    ) -> bool:
        """
...
def create_aggregation_contract(
    level: str, abort_on_violation: bool = True
) -> BaseAggregationContract:
    """
    ...
    if level.lower() not in contracts:
        raise ValueError(
            f"Invalid aggregation level: {level}. Must be one of {list(contracts.keys())}"
        )
