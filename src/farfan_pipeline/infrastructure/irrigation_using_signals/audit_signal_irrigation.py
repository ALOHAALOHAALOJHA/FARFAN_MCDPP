...
class SynchronizationAuditor:
    ...
    def audit_synchronization(
        self, phase_execution_logs: list[dict[str, Any]]
        ...
        for injection in signal_injections:
            ...
            for match in pattern_matches:
                ...
                if match_time < inj_time:
                    self.synchronization_issues.append(
...
class UtilityAuditor:
    ...
    def __init__(self, signal_registry: QuestionnaireSignalRegistry):
        ...
        self.evidence_producing_patterns: dict[str, set[str]] = defaultdict(
            set
    ...
    def audit_utility(self, execution_traces: list[dict[str, Any]]) -> SignalUtilizationMetrics:
        ...
        total_produced_evidence = sum(
            len(patterns) for patterns in self.evidence_producing_patterns.values()
        )
        ...
        metrics = SignalUtilizationMetrics(
            ...
            proof_chains_incomplete=len(
                [p for p in self.proof_chains.values() if not p.proof_chain]
            ),
