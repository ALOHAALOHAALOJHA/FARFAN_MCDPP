...
def setup_logging(level:  str = "INFO"):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            # logging.FileHandler('_logs/sisas.log') # Commented out as _logs might not exist
        ]
    )
...
def load_irrigation_map(csv_path: str) -> IrrigationMap:
    ...
    if not Path(csv_path).exists():
        # Return empty map if file not found for now to allow basic testing
...
def generate_contracts(args):
    ...
    try:
        ...
        logger.error("generate_contracts script module not found")
        return
