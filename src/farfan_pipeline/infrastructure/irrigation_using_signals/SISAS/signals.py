...
if TYPE_CHECKING:
    from cross_cutting_infrastructure.irrigation_using_signals.SISAS.signal_resolution import Signal
...
class KeywordIrrigator:
    ...
    def inject_keywords(signal_pack: SignalPack, keywords: list[str]) -> SignalPack:
        ...
        Args:
            signal_pack: Original SignalPack
            keywords: List of keywords to inject
            
        Returns:
            New SignalPack with keywords added
        """
...
class SignalTracer:
    ...
    def trace_signal_mutation(
        ...
        Args:
            signal_pack: The signal pack being mutated
        ...
        Raises:
            AssertionError: If audit trace fails
        """
        import getpass
    ...
    def trace_signal_access(
        self,
        signal_pack: SignalPack,
        accessor: str,
        purpose: str,
    ) -> SignalAuditRecord:
        """Trace signal pack access for auditing."""
        return self.trace_signal_mutation(
            signal_pack,
            f"ACCESS:{accessor}",
            {"purpose": purpose},
        )
    ...
    def get_operations_for_signal(self, signal_pack_id: str) -> list[SignalAuditRecord]:
        """Get all operations for a specific signal pack."""
        return [r for r in self._audit_log if r.signal_pack_id == signal_pack_id]
