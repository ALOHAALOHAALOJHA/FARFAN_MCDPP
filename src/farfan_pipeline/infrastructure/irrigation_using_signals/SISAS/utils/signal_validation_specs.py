...
class ValidationSpecifications:
    ...
    def validate_evidence(self, evidence: dict[str, Any]) -> ValidationResult:
        ...
        required_passed = all(results.get(vtype, False) for vtype in self.required_validations)
...
def extract_validation_specifications(
    ...
    for val_type_str, val_config in validations.items():
        ...
        if val_type_str not in [
            ...
            logger.warning(
                "unknown_validation_type", question_id=question_id, validation_type=val_type_str
            )
            continue
...
def _extract_validation_value(evidence: dict[str, Any], validation_type: ValidationType) -> float:
    ...
    if validation_type == "completeness_check":
        ...
        expected_indicators = evidence.get("criteria", {}).get(
            "expected_indicators", DEFAULT_EXPECTED_INDICATORS
        )
        ...
        # Get expected count from criteria, or use default
        expected_keywords = evidence.get("criteria", {}).get(
