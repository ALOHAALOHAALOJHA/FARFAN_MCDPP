...
class InterdependencyRule:
    ...
    source_signal: str
    target_signal: str
    condition: str  # e.g., "requires", "conflicts_with", "enhances"
...
class InterdependencyValidator:
    ...
    # Canonical interdependency rules
    CANONICAL_RULES: list[dict[str, Any]] = [
        ...
        {
            "rule_id": "IDR-003",
            "description": "Quantitative claims require source attribution",
            "source_signal": "quantitative_claim",
            "target_signal": "official_source",
            "condition": "requires",
            "severity": "warning",
        },
        {
            "rule_id": "IDR-004",
            "description": "Causal claims require mechanism evidence",
            "source_signal": "causal_link",
            "target_signal": "mechanism_evidence",
            "condition": "requires",
            "severity": "warning",
        },
    ]
    ...
    def run_interdependency_rules(
        ...
        Args:
            signal_context: Dictionary of available signals/evidence
            question_id: Question being validated
            
        Returns:
            List of InterdependencyFinding objects
            
        Raises:
            AssertionError: If not all rules are executed
        """
        ...
        logger.debug(
            "interdependency_rules_executed",
            question_id=question_id,
            rules_executed=executed_rules,
            findings_count=sum(1 for f in findings if f.triggered),
        )
    ...
    def _validate_single_rule(
        ...
        elif rule.condition == "conflicts_with":
            if has_source and has_target:
                triggered = True
                message = f"Signal '{rule.source_signal}' conflicts with '{rule.target_signal}'"
        elif rule.condition == "enhances":
            if has_source and not has_target:
                message = f"Signal '{rule.source_signal}' would be enhanced by '{rule.target_signal}'"
                # Not triggered as error, just informational
...
class BlockingRuleDefinition:
    ...
    rule_id: str
    description: str
    condition_type: str  # e.g., "confidence_below", "missing_required", "contradiction"
...
class SignalGateway:
    ...
    CANONICAL_BLOCKING_RULES: list[dict[str, Any]] = [
        ...
        {
            "rule_id": "BLK-002",
            "description": "Block if required element missing",
        ...
        {
            "rule_id": "BLK-004",
            "description": "Flag if validation error count exceeds limit",
            "condition_type": "validation_error_count",
            "threshold": 5,
            "action": "flag",
        },
    ]
    ...
    def apply_blocking_rules(
        ...
        Args:
            signal_context: Context containing signal data and metrics
            validation_report: Optional validation report with findings
            
        Raises:
            PermissionError: If any blocking rule triggers
        """
        ...
        for rule in self._blocking_rules:
            ...
            if triggered:
                violation = BlockingRuleViolation(
                    ...
                    action=rule.action,
                    affected_elements=self._get_affected_elements(rule, signal_context),
                    timestamp=time.time(),
                )
    ...
    def _evaluate_blocking_condition(
        ...
        elif rule.condition_type == "missing_required":
            required = signal_context.get("required_elements", [])
            present = signal_context.get("present_elements", [])
            return any(r not in present for r in required)
        
        elif rule.condition_type == "contradiction_detected":
            contradictions = signal_context.get("contradictions", [])
            return len(contradictions) > 0
        
        elif rule.condition_type == "validation_error_count":
    ...
    def _get_affected_elements(
        ...
        """Get list of affected elements for a blocking violation."""
        if rule.condition_type == "missing_required":
            required = signal_context.get("required_elements", [])
            present = signal_context.get("present_elements", [])
            return [r for r in required if r not in present]
        elif rule.condition_type == "contradiction_detected":
            return signal_context.get("contradictions", [])[:5]
        return []
