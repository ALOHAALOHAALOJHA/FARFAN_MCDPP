...
def extract_method_metadata(
    question_data: dict[str, Any], question_id: str
    ...
    if not methods:
        logger.warning("method_metadata_extraction_empty", question_id=question_id)
        return _create_empty_metadata()
    ...
    priority_groups_frozen = {p: tuple(ms) for p, ms in priority_groups.items()}
    ...
    # Execution order
    execution_order = tuple(f"{m.class_name}.{m.method_name}" for m in methods)
...
def should_execute_method(method_metadata: MethodMetadata, context: dict[str, Any]) -> bool:
...
def get_adaptive_execution_plan(
    metadata: MethodExecutionMetadata, context: dict[str, Any]
    ...
    """
    selected_methods = [m for m in metadata.methods if should_execute_method(m, context)]
