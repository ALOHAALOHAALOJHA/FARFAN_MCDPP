    ...
    for q_spec in questions:
        ...
        for page_idx, page_text in enumerate(pages_lower):
            ...
            for pat_spec in patterns:
                pattern = pat_spec.get('pattern', '') if isinstance(pat_spec, dict) else str(pat_spec)
                if not pattern:
                    continue
                try:
                    if re.search(pattern, page_text, re.IGNORECASE):
                        weight = pat_spec.get('confidence_weight', 0.8) if isinstance(pat_spec, dict) else 0.8
                        score += weight * 0.4
        ...
        if anchor.page_ranges:
            anchor.confidence = sum(pr.relevance for pr in anchor.page_ranges) / len(anchor.page_ranges)
...
def _extract_keywords_from_question(question_text: str) -> list[str]:
    ...
    stopwords = {
        ...
        'en', 'por', 'para', 'con', 'sin', 'sobre', 'entre', 'que', 'cual', 'como',
        'se', 'su', 'sus', 'este', 'esta', 'estos', 'estas', 'ese', 'esa', 'esos',
        'qué', 'cómo', 'cuál', 'cuáles', 'dónde', 'cuándo', 'cuánto', 'cuánta',
        'son', 'es', 'está', 'están', 'tiene', 'tienen', 'hay', 'debe', 'deben',
        'plan', 'define', 'establece', 'incluye', 'presenta', 'describe', 'menciona'
    }
