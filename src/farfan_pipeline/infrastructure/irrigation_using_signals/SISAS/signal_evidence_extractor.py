...
def extract_structured_evidence(
    ...
    for element_spec in expected_elements:
        ...
        if isinstance(element_spec, str):
            ...
            element_type = (
                element_spec.type if hasattr(element_spec, "type") else element_spec.get("type", "")
            )
        ...
        if is_required and found_count == 0:
            ...
            logger.debug("required_element_missing", element_type=element_type)
    ...
    completeness = compute_completeness(evidence=evidence, expected_elements=expected_elements)
    ...
    for pattern_spec in all_patterns:
        ...
        for alt in alternatives:
            ...
            try:
                for match in re.finditer(regex_pattern, text, re.IGNORECASE):
                    ...
                    match_item = {
                        ...
                        "lineage": {
                            "pattern_id": pattern_id,
                            "pattern_text": (
                                pattern_str[:50] + "..." if len(pattern_str) > 50 else pattern_str
                            ),
                ...
                logger.warning("pattern_regex_error", pattern_id=pattern_id, error=str(e))
                continue
...
def _infer_pattern_categories_for_element(element_type: str) -> list[str] | None:
    ...
    # Temporal elements
    if any(
        kw in element_type.lower()
        for kw in ["temporal", "año", "años", "plazo", "cronograma", "series"]
    ):
    ...
    if any(
        kw in element_type.lower()
        for kw in ["cuantitativo", "indicador", "meta", "brecha", "baseline"]
    ):
        return ["QUANTITATIVE", "GENERAL"]

    # Geographic/territorial
    if any(
        kw in element_type.lower() for kw in ["territorial", "cobertura", "geographic", "región"]
    ):
...
def _is_pattern_relevant_to_element(
    pattern_str: str, element_type: str, pattern_spec: dict[str, Any]
) -> bool:
    """
...
def compute_completeness(
    evidence: dict[str, list[dict[str, Any]]], expected_elements: list[dict[str, Any]]
) -> float:
    """
    ...
    for element_spec in expected_elements:
        ...
        is_required = (
            element_spec.required
            if hasattr(element_spec, "required")
            else element_spec.get("required", False)
        )
        minimum_count = (
            element_spec.minimum
            if hasattr(element_spec, "minimum")
            else element_spec.get("minimum", 0)
        )
