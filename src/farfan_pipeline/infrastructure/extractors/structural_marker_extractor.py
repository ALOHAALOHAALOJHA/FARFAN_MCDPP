...
class StructuralMarkerExtractor(PatternBasedExtractor):
    ...
    def _build_patterns(self):
        ...
        self._compiled_table_indicators = [re.compile(p, re.IGNORECASE) for p in self.table_indicator_patterns]
        self._compiled_header_patterns = [(re.compile(p, re.MULTILINE), level) for p, level in self.header_patterns]
    ...
    def extract(self, text: str, context: Optional[Dict] = None) -> ExtractionResult:
        ...
        result = ExtractionResult(
            ...
            metadata={
                ...
                "section_headers": by_type.get("section_header", 0) + by_type.get("subsection_header", 0),
            }
        )
    ...
    def _detect_headers(self, text: str) -> List[StructuralMarker]:
        ...
        for pattern, level in self._compiled_header_patterns:
            for match in pattern.finditer(text):
                ...
                structure_type = StructureType.SECTION_HEADER if level == 1 else StructureType.SUBSECTION_HEADER
