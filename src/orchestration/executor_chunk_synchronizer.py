...
def build_join_table(
    ...
    chunk_id_usage: dict[str, str] = {}  # Track chunk usage to detect duplicates
    ...
    for contract in contracts:
        ...
        chunk_id = (
            getattr(chunk, "chunk_id", None) or
            (chunk.get("chunk_id") if isinstance(chunk, dict) else None) or
            f"{policy_area_id}-{dimension_id}"
        )
    ...
    logger.info(
        f"✓ JOIN table built successfully: {len(bindings)} bindings, "
        f"{len(chunk_id_usage)} unique chunks"
    )
...
def validate_uniqueness(bindings: list[ExecutorChunkBinding]) -> None:
    ...
    2. Each chunk_id appears exactly once
    ...
    logger.debug("✓ 1:1 mapping invariants validated")
...
def generate_verification_manifest(
    ...
    invariants_validated = {
        "one_to_one_mapping": len(bindings) == len(set(b.executor_contract_id for b in bindings)),
