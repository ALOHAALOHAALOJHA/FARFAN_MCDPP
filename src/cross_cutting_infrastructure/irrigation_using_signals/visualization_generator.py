...
class SankeyDiagramGenerator:
    ...
    def add_node(self, node_id: str, label: str, value: int = 0, category: str = "") -> None:
        ...
        if node_id not in self.node_map:
            ...
            self.nodes.append(SignalFlowNode(id=node_id, label=label, value=value, category=category))
        else:
...
class HeatmapGenerator:
    ...
    def __init__(self):
        self.data: dict[tuple[str, str], float] = defaultdict(float)  # (phase, policy_area) -> value
...
class VisualizationOrchestrator:
    ...
    def _generate_state_machine(self) -> Path:
        ...
        generator.add_transition("loading_questionnaire", "extracting_signals", "Questionnaire Loaded")
