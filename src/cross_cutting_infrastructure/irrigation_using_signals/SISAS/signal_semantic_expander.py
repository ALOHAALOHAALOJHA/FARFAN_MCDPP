...
def expand_pattern_semantically(
    ...
    for idx, synonym in enumerate(synonyms, 1):
        ...
        logger.debug(
            ...
            synonym=synonym,
            variant_pattern=variant_pattern[:50] + "..." if len(variant_pattern) > 50 else variant_pattern
        )
...
def expand_all_patterns(
    ...
    if enable_logging:
        ...
        logger.info(
            ...
            expansion_rate_pct=round((expansion_stats['patterns_with_expansion'] / expansion_stats['original_count'] * 100), 1) if expansion_stats['original_count'] > 0 else 0.0,
            total_variants_generated=expansion_stats['variant_count'],
            avg_variants_per_expanded_pattern=round(expansion_stats['variant_count'] / expansion_stats['patterns_with_expansion'], 2) if expansion_stats['patterns_with_expansion'] > 0 else 0.0,
