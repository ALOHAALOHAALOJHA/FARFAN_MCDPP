...
def example_1_basic_calculation(verbose: bool = False) -> None:
    ...
    result = aggregator.aggregate(subject="ExampleMethod", layer_scores=layer_scores)
...
def example_2_with_interactions(verbose: bool = False) -> None:
    ...
    config = ChoquetConfig(
        linear_weights={"@b": 0.35, "@chain": 0.30, "@q": 0.25},
        interaction_weights={("@b", "@chain"): 0.10, ("@chain", "@q"): 0.05},
        normalize_weights=False,
        validate_boundedness=True,
    )

    print("\nLayer Scores:")
    layer_scores = {"@b": 0.80, "@chain": 0.70, "@q": 0.85}
    for layer, score in layer_scores.items():
        print(f"  {layer:<10} {score:.2f}")

    aggregator = ChoquetAggregator(config)
    result = aggregator.aggregate(subject="ExampleMethodWithSynergies", layer_scores=layer_scores)
    ...
    print(
        f"  Interaction (@chain, @q):   {result.breakdown.per_interaction_contributions[('@chain', '@q')]:.4f}"
    )
...
def example_3_normalization(verbose: bool = False) -> None:
    ...
    config = ChoquetConfig(
        linear_weights={"@b": 4.0, "@chain": 3.0, "@q": 2.0, "@d": 1.0},
        interaction_weights={("@b", "@chain"): 1.5, ("@q", "@d"): 0.5},
        normalize_weights=True,
        validate_boundedness=True,
    )

    print("\nLayer Scores:")
    layer_scores = {"@b": 0.90, "@chain": 0.85, "@q": 0.75, "@d": 0.80}
    for layer, score in layer_scores.items():
        print(f"  {layer:<10} {score:.2f}")
    ...
    result = aggregator.aggregate(subject="HighPerformingMethod", layer_scores=layer_scores)
...
def example_4_boundary_cases(verbose: bool = False) -> None:
    ...
    aggregator = ChoquetAggregator(config)
    result = aggregator.aggregate("WorstCase", {"@b": 0.0, "@chain": 0.0, "@q": 0.0})
    ...
    aggregator = ChoquetAggregator(config)
    result = aggregator.aggregate("PerfectCase", {"@b": 1.0, "@chain": 1.0, "@q": 1.0})
    print(f"  Cal(I) = {result.calibration_score:.4f}")
    ...
    aggregator = ChoquetAggregator(config)
    result = aggregator.aggregate("DominantLayer", {"@b": 0.50, "@chain": 1.00, "@q": 1.00})
    print(f"  Cal(I) = {result.calibration_score:.4f}")
    ...
    aggregator = ChoquetAggregator(config)
    result = aggregator.aggregate("AsymmetricInteraction", {"@b": 1.0, "@chain": 0.0})
    print(f"  Cal(I) = {result.calibration_score:.4f}")
    ...
    aggregator = ChoquetAggregator(config)
    result = aggregator.aggregate("WeightedAverage", {"@b": 0.7, "@chain": 0.6, "@q": 0.8})
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run Choquet Aggregator examples")
    parser.add_argument(
        "--example", type=int, choices=[1, 2, 3, 4, 5], help="Run specific example (1-5)"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Enable verbose output with detailed breakdowns"
    )
