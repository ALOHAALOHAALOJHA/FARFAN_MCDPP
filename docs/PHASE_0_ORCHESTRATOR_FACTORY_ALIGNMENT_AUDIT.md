...
### C-01: Orchestrator Constructor Receives Extra Parameters from Factory
...
**Issue**: Factory passes `validation_constants` and `signal_registry` as kwargs, but Orchestrator signature doesn't accept them:

```python
# Orchestrator signature (orchestrator.py:1297-1309):
def __init__(
    self,
...
# Factory passes (phase2_10_00_factory.py:1111-1118):
orchestrator = Orchestrator(
    ...
    validation_constants=validation_constants,    # ✗ REJECTED - not in signature
    signal_registry=self._signal_registry,        # ✗ REJECTED - not in signature
)
```

**Impact**: `TypeError: __init__() got unexpected keyword arguments 'validation_constants', 'signal_registry'`

**Remediation**: Remove extra kwargs from Factory (signal_registry is already in method_executor, validation_constants should go in processor_bundle):
```python
# In phase2_10_00_factory.py:_build_orchestrator()
orchestrator = Orchestrator(
    method_executor=self._method_executor,  # Already contains signal_registry
    questionnaire=self._canonical_questionnaire,
    executor_config=executor_config,
    processor_bundle=self,  # Pass factory as bundle for enriched_packs access
    # Remove: validation_constants, signal_registry
)
```
