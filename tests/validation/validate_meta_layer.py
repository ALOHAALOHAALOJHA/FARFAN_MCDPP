...
def validate():
    try:
        ...
        print(
            f"✓ Default config created: w_transp={config.w_transparency}, w_gov={config.w_governance}, w_cost={config.w_cost}"
        )
        ...
        m_transp_07 = evaluator.evaluate_transparency(
            {
                "formula_export": "Cal(I) = expanded Choquet integral",
                "trace": "Phase 1 step execution trace complete",
                "logs": None,
            },
            None,
        )
        ...
        m_transp_04 = evaluator.evaluate_transparency(
            {"formula_export": None, "trace": "Phase 0 step execution method trace", "logs": None},
            None,
        )
        ...
        m_cost_1 = evaluator.evaluate_cost({"execution_time_s": 0.5, "memory_usage_mb": 256.0})
        print(f"  Fast (<1s, ≤512MB): {m_cost_1} (expected 1.0)")
        assert m_cost_1 == 1.0, f"Should be 1.0, got {m_cost_1}"

        m_cost_08 = evaluator.evaluate_cost({"execution_time_s": 3.0, "memory_usage_mb": 400.0})
        print(f"  Acceptable (<5s, ≤512MB): {m_cost_08} (expected 0.8)")
        assert m_cost_08 == 0.8, f"Should be 0.8, got {m_cost_08}"

        m_cost_05 = evaluator.evaluate_cost({"execution_time_s": 6.0, "memory_usage_mb": 256.0})
        print(f"  Poor (≥5s): {m_cost_05} (expected 0.5)")
        assert m_cost_05 == 0.5, f"Should be 0.5, got {m_cost_05}"

        m_cost_0 = evaluator.evaluate_cost({"execution_time_s": -1.0, "memory_usage_mb": 256.0})
        ...
        test_cost: CostMetrics = {"execution_time_s": 0.5, "memory_usage_mb": 256.0}
        ...
        assert (
            result["m_transparency"] == 1.0
        ), f"m_transp should be 1.0, got {result['m_transparency']}"
        ...
        result2 = evaluator.evaluate(
            {
                "formula_export": "x_@m calibration formula expanded",
                "trace": "Phase 1 step execution method trace complete",
                "logs": None,
            },  # 0.7
