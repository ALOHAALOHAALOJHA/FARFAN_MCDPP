...
def test_phase_timeout_error_basic():
    ...
    error = PhaseTimeoutError(phase_id=2, phase_name="FASE 2 - Micro Preguntas", timeout_s=600.0)
...
def test_phase_timeout_error_with_elapsed():
    ...
    error = PhaseTimeoutError(
        phase_id=7, phase_name="FASE 7 - Macro", timeout_s=60.0, elapsed_s=61.5
    )
...
async def test_execute_phase_with_timeout_success():
    ...
    result = await execute_phase_with_timeout(
        phase_id=0, phase_name="Test Phase", handler=fast_handler, timeout_s=5.0
    )
...
async def test_execute_phase_with_timeout_raises_timeout():
    ...
    with pytest.raises(PhaseTimeoutError) as exc_info:
        await execute_phase_with_timeout(
            phase_id=2, phase_name="Slow Phase", handler=slow_handler, timeout_s=0.5
        )
...
async def test_execute_phase_with_timeout_sync_function():
    ...
    result = await execute_phase_with_timeout(
        phase_id=1, phase_name="Sync Phase", handler=sync_handler, timeout_s=5.0
    )
...
def test_get_phase_timeout_prod_mode(runtime_config_prod):
    ...
    with patch("farfan_pipeline.orchestration.core_orchestrator.MethodExecutor"):
        with patch("farfan_pipeline.orchestration.core_orchestrator.get_questionnaire_provider"):
            orchestrator = Mock()
...
def test_get_phase_timeout_dev_mode(runtime_config_dev):
    ...
    with patch("farfan_pipeline.orchestration.core_orchestrator.RuntimeMode", RuntimeMode):
        from farfan_pipeline.orchestration.core_orchestrator import Orchestrator
...
async def test_integration_partial_progress_extraction():
    ...
    with pytest.raises(PhaseTimeoutError):
        await execute_phase_with_timeout(
            phase_id=2, phase_name="Partial Phase", handler=partial_handler, timeout_s=0.3
        )
...
def test_build_execution_manifest_timeout_prod_mode(runtime_config_prod):
    ...
    from farfan_pipeline.orchestration.core_orchestrator import PhaseResult
...
def test_build_execution_manifest_timeout_dev_mode(runtime_config_dev):
    ...
    orchestrator._phase_status = {
        i: "not_started" if i > 2 else ("failed" if i == 2 else "completed") for i in range(11)
    }
    orchestrator.abort_signal = Mock()
    orchestrator.abort_signal.is_aborted = Mock(return_value=True)
    orchestrator.abort_signal.get_reason = Mock(return_value="Phase 2 timed out")
...
def test_regression_all_sync_phases_use_timeout():
    ...
    sync_phases_in_fases = {
        phase_id for phase_id, mode, _, _ in Orchestrator.FASES if mode == "sync"
    }
...
async def test_regression_timeout_cannot_be_bypassed():
    ...
    with pytest.raises(PhaseTimeoutError):
        await execute_phase_with_timeout(
            phase_id=999, phase_name="Sneaky Phase", handler=sneaky_handler, timeout_s=0.5
        )
...
async def test_execute_phase_with_timeout_zero_timeout():
    ...
    with pytest.raises(PhaseTimeoutError):
        await execute_phase_with_timeout(
            phase_id=0, phase_name="Zero Timeout", handler=instant_handler, timeout_s=0.0
        )
...
def test_phase_timeout_error_string_phase_id():
    ...
    error = PhaseTimeoutError(phase_id="2", phase_name="Phase Two", timeout_s=100.0)
...
async def test_execute_phase_with_timeout_exception_during_execution():
    ...
    with pytest.raises(ValueError, match="Handler failed"):
        await execute_phase_with_timeout(
            phase_id=5, phase_name="Failing Phase", handler=failing_handler, timeout_s=5.0
        )
