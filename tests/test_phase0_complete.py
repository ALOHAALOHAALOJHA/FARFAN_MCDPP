...
class TestPhase0InputContract:
    ...
    def test_phase0_input_validator_empty_pdf_path(self):
        ...
        with pytest.raises(
            Exception, match=r"(pdf_path cannot be empty|String should have at least 1 character)"
        ):
    ...
    def test_phase0_input_validator_empty_run_id(self):
        ...
        with pytest.raises(
            Exception, match=r"(run_id cannot be empty|String should have at least 1 character)"
        ):
    ...
    def test_phase0_input_validator_invalid_run_id_characters(self):
        ...
        for char in invalid_chars:
            with pytest.raises(Exception, match="invalid characters"):
                Phase0InputValidator(pdf_path="/tmp/test.pdf", run_id=f"test{char}run")
...
class TestCanonicalInputContract:
    ...
    def test_canonical_input_validator_invalid_sha256_length(self):
        ...
        with pytest.raises(
            Exception, match=r"(must be 64 characters|String should have at least 64 characters)"
        ):
            CanonicalInputValidator(
                document_id="test",
                run_id="test_run",
