...
def mock_signal_pack() -> MagicMock:
    ...
    pack.question_patterns = {
        "Q230": [
            MagicMock(
                model_dump=lambda: {"id": "PAT-Q230-000", "pattern": "PND|Plan Nacional"}
            ),
            MagicMock(
                model_dump=lambda: {"id": "PAT-Q230-001", "pattern": "ODS|Objetivos"}
            ),
        ]
    }
    return pack
...
class TestV4ContractHydration:
    ...
    def test_hydrates_question_text(
        ...
        assert result["question_context"]["question_text"] == \
    ...
    def test_hydrates_identity_fields(
        self, hydrator: ContractHydrator, v4_contract: Dict
    ) -> None:
    ...
    def test_adds_hydration_metadata(
        self, hydrator: ContractHydrator, v4_contract: Dict
    ) -> None:
...
class TestV3ContractPassthrough:
    ...
    def test_v3_not_modified(
        self, hydrator: ContractHydrator, v3_contract: Dict
    ) -> None:
        """V3 contract is returned unchanged."""
        result = hydrator.hydrate(v3_contract)
        
        assert "_hydration_metadata" not in result or \
               not result.get("_hydration_metadata", {}).get("was_hydrated")
    
    def test_v3_preserves_all_fields(
        self, hydrator: ContractHydrator, v3_contract: Dict
    ) -> None:
        ...
        assert result["question_context"]["expected_elements"] == \
...
class TestHydrationIdempotence:
    ...
    def test_already_hydrated_detected(
        self, hydrator: ContractHydrator, v4_contract: Dict
    ) -> None:
...
class TestOverrides:
    ...
    def test_override_question_text(
        self, hydrator: ContractHydrator, v4_contract: Dict
    ) -> None:
        """Override question_text takes precedence."""
        v4_contract["question_context"]["overrides"] = {
    ...
    def test_override_expected_elements(
        ...
        v4_contract["question_context"]["overrides"] = {
            "expected_elements": custom_elements
        }
...
class TestErrorHandling:
    ...
    def test_missing_question_id_raises(
        self, hydrator: ContractHydrator
    ) -> None:
    ...
    def test_signal_fetch_failure_raises(
        ...
        mock_signal_registry.get_micro_answering_signals.side_effect = \
...
class TestValidation:
    ...
    def test_validate_incomplete_contract(
        self, hydrator: ContractHydrator
    ) -> None:
