...
def test_metrics_not_only_in_logs(tmp_path: Path) -> None:
    ...
    assert written_files[
        "phase_metrics"
    ].exists(), "REGRESSION: phase_metrics.json was not written to disk"
    assert written_files[
        "resource_usage"
    ].exists(), "REGRESSION: resource_usage.jsonl was not written to disk"
    assert written_files[
        "latency_histograms"
    ].exists(), "REGRESSION: latency_histograms.json was not written to disk"

    # Verify files have content (not empty)
    assert (
        written_files["phase_metrics"].stat().st_size > 0
    ), "REGRESSION: phase_metrics.json is empty"
    assert (
        written_files["latency_histograms"].stat().st_size > 0
...
def test_orchestrator_export_metrics_is_callable() -> None:
    ...
    assert callable(
        getattr(Orchestrator, "export_metrics")
...
def test_metrics_persistence_in_ci_artifacts_directory(tmp_path: Path) -> None:
    ...
    assert expected_phase_metrics.exists(), f"REGRESSION: CI cannot find {expected_phase_metrics}"
    assert expected_resource_usage.exists(), f"REGRESSION: CI cannot find {expected_resource_usage}"
    assert (
        expected_latency_histograms.exists()
...
def test_metrics_files_have_required_content() -> None:
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        assert (
            "latency_histogram" in histogram_data["0"]
        ), "REGRESSION: latency_histograms.json missing latency_histogram"

        hist = histogram_data["0"]["latency_histogram"]
        for percentile in ["p50", "p95", "p99"]:
            assert percentile in hist, f"REGRESSION: latency_histograms.json missing {percentile}"
...
def test_metrics_persistence_is_deterministic() -> None:
    ...
    assert content1 == content2, "REGRESSION: Metrics persistence is not deterministic"
...
def test_missing_metrics_files_detectable() -> None:
    ...
    assert len(missing_files) == 3, "REGRESSION: Cannot detect missing metrics files"
...
def test_metrics_schema_validation_prevents_invalid_data() -> None:
    ...
    assert len(errors) == 0, f"REGRESSION: Valid metrics failed validation: {errors}"

    # Invalid metrics should fail
    invalid_metrics = {"timestamp": "2024-01-01T00:00:00.000000", "phase_metrics": "not a dict"}

    errors = validate_metrics_schema(invalid_metrics)
    assert len(errors) > 0, "REGRESSION: Invalid metrics passed validation"
