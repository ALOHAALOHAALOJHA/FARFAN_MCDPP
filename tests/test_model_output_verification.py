...
class TestPhase0Models:
    ...
    def test_phase0_input_instantiation(self):
        ...
        input_data = Phase0Input(
            pdf_path=Path("test.pdf"), run_id="test_run_001", questionnaire_path=None
        )
...
class TestPhase1SubphaseModels:
    ...
    def test_sp13_validation_result(self):
        ...
        validation = ValidationResult(
            status="VALID", violations=[], checked_count=60, passed_count=60
        )
...
class TestCPPModels:
    ...
    def test_canon_policy_package_assembly(self):
        ...
        integrity = IntegrityIndex(
            blake2b_root="a" * 64, chunk_hashes=("abc123",), timestamp="2025-12-10T17:00:00Z"
        )
...
class TestConstitutionalInvariants:
    ...
    def test_60_chunk_invariant_sp11(self):
        ...
        assert (
            len(smart_chunks) == 60
        ), f"SP11 must produce exactly 60 smart chunks, got {len(smart_chunks)}"
    ...
    def test_60_chunk_invariant_cpp(self):
        ...
        # CONSTITUTIONAL INVARIANT
        assert (
            chunk_graph.chunk_count == 60
        ), f"CPP must have exactly 60 chunks, got {chunk_graph.chunk_count}"
