...
class TestExecutorCalibrationIntegration:
    ...
    def test_all_executors_count(self):
        ...
        assert len(ALL_EXECUTOR_IDS) == 30, f"Expected 30 executors, found {len(ALL_EXECUTOR_IDS)}"
    ...
    def test_instrument_executor_basic(self):
        ...
        context = {"document_text": "Test document", "policy_area": "PA01", "unit_id": "test_unit"}
    ...
    def test_quality_score_calculation(self):
        ...
        for executor_id in ALL_EXECUTOR_IDS[:5]:  # Test first 5
            ...
            context = CalibrationContext(
                question=question, dimension=dimension, executor_class=executor_id
            )

            quality_score, layer_scores = integration.calculate_quality_score(
                f"farfan_pipeline.core.orchestrator.executors.{executor_id}", context
            )
    ...
    def test_executor_config_loading(self):
        ...
        config = get_executor_config(
            executor_id="D3_Q2_TargetProportionalityAnalyzer", dimension="D3", question="Q2"
        )
    ...
    def test_no_hardcoded_calibration_values(self):
        ...
        for pattern in forbidden_patterns:
            assert pattern not in content, f"Found hardcoded calibration pattern: {pattern}"
    ...
    def test_calibration_parametrization_separation(self):
        ...
        def check_dict_recursive(d: Dict, path: str = "") -> None:
            for key, value in d.items():
                ...
                assert (
                    key not in forbidden_runtime_keys
    ...
    def test_layer_score_retrieval(self):
        ...
        integration = CalibrationIntegration()
        context = CalibrationContext(
            question="Q2", dimension="D3", executor_class="D3_Q2_TargetProportionalityAnalyzer"
        )
    ...
    def test_choquet_aggregation(self):
        ...
        linear_sum = sum(
            LAYER_WEIGHTS.get(layer, 0.0) * score for layer, score in layer_scores.items()
        )
        ...
        assert (
            abs(quality_score - expected_score) < 0.01
