...
def create_dummy_manifest(
    contract_type, ingestion_prior, phase2_prior, phase2_veto, decisions=None
):
    uoa = UnitOfAnalysis(
    ...
    def create_layer(phase, prior, veto):
        params = [
            CalibrationParameter(
                ...
                value=prior,
                unit="dimensionless",
                bounds=ClosedInterval(0.0, 10.0),
                rationale="Test",
            ...
            CalibrationParameter(
                ...
                value=2000.0,
                unit="tokens",
                bounds=ClosedInterval(100.0, 10000.0),
                rationale="Test",
                evidence=EvidenceReference("docs/README.md", "a" * 40, "desc"),
                calibrated_at=now,
                expires_at=future,
...
def test_prior_strength_out_of_bounds_triggers_veto() -> None:
    ...
    manifest = create_dummy_manifest(
        contract_type="TYPE_A", ingestion_prior=0.05, phase2_prior=0.05, phase2_veto=0.05
    )
...
def test_veto_threshold_mismatch_triggers_veto() -> None:
    ...
    # 0.99 is invalid
    manifest = create_dummy_manifest(
        contract_type="TYPE_A", ingestion_prior=1.0, phase2_prior=1.0, phase2_veto=0.99
    )
    auditor = CalibrationAuditor()
    audit_result = auditor.audit(manifest)
    assert not audit_result.passed
...
def test_drift_detection_flags_severe_coverage_drift() -> None:
    auditor = CalibrationAuditor()
    manifest = create_dummy_manifest(
        contract_type="TYPE_A", ingestion_prior=1.0, phase2_prior=1.0, phase2_veto=0.05
    )
