...
class TestCL04PDETEnrichment:
    ...
    def test_cl04_metadata_has_pdet_enrichment(self, cl04_metadata):
        ...
        assert "pdet_enrichment" in cl04_metadata, "CL04 metadata must include pdet_enrichment config"
    ...
    def test_pa09_enrichment_request_success(
        ...
        assert all(result.gate_status.values()), f"All gates should pass. Status: {result.gate_status}"
    ...
    def test_gate_1_scope_validation(
    ...
    def test_gate_3_capability_validation(
        self, orchestrator, valid_pa09_consumer_scope
    ):
        ...
        # Gate 3 should fail
        assert result.gate_status["gate_3_capability"] is False, "Gate 3 should fail for insufficient capabilities"
        assert not result.success, "Enrichment should fail"

    def test_enrichment_report_generation(self, orchestrator, valid_pa09_consumer_scope, valid_capabilities):
