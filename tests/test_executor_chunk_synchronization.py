...
def sample_contracts():
    ...
    for i in range(1, EXPECTED_CONTRACT_COUNT + 1):
        ...
        contract = {
            ...
            "question_context": {
                "patterns": [
                    {"id": f"PAT-Q{i:03d}-{j:03d}", "pattern": f"test_{j}"} for j in range(3)
                ]
            },
            "signal_requirements": {"mandatory_signals": ["signal_1", "signal_2", "signal_3"]},
        }
...
def test_build_join_table_duplicate_chunk(sample_contracts):
    ...
    # Add duplicate chunk
    chunks.append(
        {"policy_area_id": "PA01", "dimension_id": "DIM01", "chunk_id": "PA01-DIM01-duplicate"}
    )
...
def test_full_synchronization_workflow(sample_contracts, sample_chunks):
    ...
    for binding in bindings:
        ...
        binding.irrigated_signals = [{"signal_type": sig} for sig in binding.expected_signals]
...
def test_malformed_contract():
    ...
    contracts = [
        {"identity": {}} for _ in range(EXPECTED_CONTRACT_COUNT)
