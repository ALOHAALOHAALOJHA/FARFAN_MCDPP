...
def test_dura_lex_01_all_operations_must_be_auditable():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch(
            "farfan_pipeline.phases.Phase_zero.phase0_90_01_verified_pipeline_runner.RuntimeConfig"
        ) as mock_config:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch(
            ...
            runner = VerifiedPipelineRunner(
                pdf_path, Path(tmpdir) / "artifacts", Path(tmpdir) / "q.json"
            )
...
def test_dura_lex_15_all_decisions_must_leave_trace():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch(
            ...
            trace = {
                "input_pdf_hash": (
                    runner.input_pdf_sha256 if hasattr(runner, "input_pdf_sha256") else None
                ),
                "questionnaire_hash": (
                    runner.questionnaire_sha256 if hasattr(runner, "questionnaire_sha256") else None
                ),
                "bootstrap_failed": runner._bootstrap_failed,
