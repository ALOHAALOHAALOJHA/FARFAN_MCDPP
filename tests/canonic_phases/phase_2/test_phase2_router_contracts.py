...
class MockExecutor:
    ...
    def run(self, payload: MockPayload) -> MockResult:
        return MockResult(
            task_id=f"task_{payload.chunk_id}", chunk_id=payload.chunk_id, success=True
        )
...
class TestRouterConstruction:
    ...
    def test_construction_with_valid_registry(
        self, valid_registry: Dict[str, Type[Executor]]
    ) -> None:
...
class TestRouterIntegration:
    ...
    def test_route_multiple_payloads_in_sequence(self, router: ArgRouter) -> None:
        ...
        payloads = [MockPayload("specialized_contract", f"chunk_{i:03d}") for i in range(10)]
