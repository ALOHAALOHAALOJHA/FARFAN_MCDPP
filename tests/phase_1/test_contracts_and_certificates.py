...
class TestContractModules:
    ...
    def test_all_contract_files_exist(self):
        ...
        for filename in contract_files:
            ...
            assert (
                size_bytes > 500
            ), f"{filename} should be substantial (>500 bytes), got {size_bytes}"
...
class TestContractStructure:
    ...
    def test_input_contract_defines_preconditions(self):
        ...
        assert (
            "def validate_phase1_input_contract" in content
        ), "validate_phase1_input_contract must exist"
    ...
    def test_output_contract_defines_postconditions(self):
        ...
        assert (
            "PHASE1_OUTPUT_POSTCONDITIONS" in content
        ), "PHASE1_OUTPUT_POSTCONDITIONS must be defined"
        ...
        # Verify validation function
        assert (
            "def validate_phase1_output_contract" in content
        ), "validate_phase1_output_contract must exist"
    ...
    def test_constitutional_contract_defines_constants(self):
        ...
        # Verify validation function
        assert (
            "def validate_constitutional_invariant" in content
        ), "validate_constitutional_invariant must exist"
...
class TestCertificates:
    ...
    def test_exactly_15_certificates(self):
        ...
        valid_certs = [
            c for c in certificates if c.name.startswith("CERTIFICATE_") and c.name.endswith(".md")
        ]

        assert (
            len(valid_certs) >= 15
        ), f"Must have at least 15 certificates, found {len(valid_certs)}: {[c.name for c in valid_certs]}"
