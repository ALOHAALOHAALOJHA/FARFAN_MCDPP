...
class TestNamingConventions:
    ...
    def test_explicit_extensions(self):
        ...
        for file_path in canonical_path.rglob("*"):
            if file_path.is_file() and not file_path.name.startswith("."):
                assert "." in file_path.name, f"NR-05 violation: File missing extension: {file_path}"
    ...
    def test_phase1_prefix_compliance(self):
        ...
        for filename in phase_specific_files:
            ...
            assert filename.startswith("phase1_"), f"NR-03 violation: File must have phase1_ prefix: {filename}"
    ...
    def test_snake_case_compliance(self):
        ...
        for file_path in canonical_path.rglob("*.py"):
            ...
            if filename != "__init__.py":
                assert filename.islower() or "_" in filename, f"NR-02 violation: Not snake_case: {filename}"
                assert not any(c.isupper() for c in filename), f"NR-02 violation: Contains uppercase: {filename}"
...
class TestPurgeVerification:
    ...
    def test_purge_command_verification(self):
        ...
        result = subprocess.run(
            [
                "find", ".", 
                "(", "-name", "*phase1*", "-o", "-name", "*Phase_one*", "-o", "-name", "*phase_one*", ")",
                "!", "-path", "*/phase_1_cpp_ingestion/*",
                "!", "-path", "*/__pycache__/*",
                "!", "-path", "*/.git/*",
                "!", "-path", "*/tests/*",  # Tests are allowed
                "!", "-name", "generate_phase1_ria.py",  # This script is allowed
            ],
            cwd=root_path,
            capture_output=True,
            text=True
        ...
        assert len(forbidden_files) == 0, f"Found phase1 files outside canonical path: {forbidden_files}"
