...
class TestResourceLimitsBypassPrevention:
    ...
    async def test_phase_execution_always_checks_limits(self) -> None:
        ...
        resource_limits = ResourceLimits(
            max_memory_mb=4096.0, max_cpu_percent=85.0, max_workers=32, min_workers=4
        )
        ...
        with patch.object(
            orchestrator, "_check_and_enforce_resource_limits", side_effect=tracking_check
        ):
    ...
    async def test_limit_checks_run_in_all_runtime_modes(self) -> None:
        ...
        for mode in [RuntimeMode.PROD, RuntimeMode.DEV, RuntimeMode.EXPLORATORY]:
            ...
            with patch.object(
                resource_limits, "check_memory_exceeded", side_effect=tracking_check_mem
            ):
                with patch.object(
                    resource_limits, "check_cpu_exceeded", side_effect=tracking_check_cpu
                ):
                    with patch.object(
                        resource_limits,
                        "get_resource_usage",
    ...
    async def test_phase2_cannot_skip_periodic_checks(self) -> None:
        ...
        with patch.dict(orchestrator.executors, {"D1-Q1": MockExecutor}):
            ...
            with patch.object(
                orchestrator, "_check_and_enforce_resource_limits", side_effect=counting_check
            ):
                await orchestrator._execute_micro_questions_async(
                    MagicMock(), {"micro_questions": test_questions}
                )
