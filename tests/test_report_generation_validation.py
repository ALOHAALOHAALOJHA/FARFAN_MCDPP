...
def test_no_stub_in_report_generator():
    ...
    assert (
        '"stub"' not in code.lower() or "# stub" in code.lower()
    ), "Unexpected stub in report_generator"
...
def test_orchestrator_phase9_not_stub():
    ...
    assert (
        "logger.warning" not in phase9_code or "stub" not in phase9_code
    ), "Phase 9 still has stub warning"
...
def test_orchestrator_phase10_not_stub():
    ...
    # Should not have stub warning
    assert not (
        "logger.warning" in phase10_code and "stub" in phase10_code
    ), "Phase 10 still has stub warning"
