...
def create_dimension_score_group(
    policy_area: str = "PA01", dimension: str = "DIM01", scores: List[float] | None = None
) -> List[ScoredResult]:
    ...
    return [
        create_valid_scored_result(
            question_id=i, policy_area=policy_area, dimension=dimension, score=score
        )
...
class TestPhase4EmptyMalformedInputs:
    ...
    def test_empty_input_list(self):
        ...
        dim_score = aggregator.aggregate_dimension(
            [], {"policy_area": "PA01", "dimension": "DIM01"}  # Empty list
        )
    ...
    def test_invalid_quality_level(self):
        ...
        assert (
            "quality_level" in str(exc_info.value).lower() or "type" in str(exc_info.value).lower()
        )
...
class TestPhase4IntermodularWiring:
    ...
    def test_entry_contract_from_phase3(self):
        ...
        is_valid, violations, metadata = contract.validate_input(results, strict_mode=False)
    ...
    def test_entry_contract_missing_traceability(self):
        ...
        is_valid, violations, metadata = validate_phase4_7_entry(results, strict_mode=False)
    ...
    def test_phase4_output_to_phase5_compatibility(self):
        ...
        dim_aggregator = DimensionAggregator(
            monolith=None, abort_on_insufficient=False, enable_sota_features=False
        )
        ...
        for dim_idx in range(1, 7):  # 6 dimensions
            results = create_dimension_score_group(
                dimension=f"DIM{dim_idx:02d}", scores=[2.0, 2.0, 2.0, 2.0, 2.0]
            )
    ...
    def test_traceability_preservation_across_phases(self):
        ...
        results = [
            create_valid_scored_result(
                question_id=qid, policy_area="PA01", dimension="DIM01", score=2.0
            )
...
class TestPhase4HermeticityAndCoverage:
        ...
        mock_monolith = {
            "blocks": {
                ...
                "niveles_abstraccion": {
                    ...
                    "dimensions": [{"dimension_id": f"DIM{i:02d}"} for i in range(1, 7)],
                },
            }
        }

        aggregator = AreaPolicyAggregator(monolith=mock_monolith, abort_on_insufficient=True)
    ...
    def test_duplicate_dimension_detection(self):
        ...
        mock_monolith = {
            "blocks": {
                "scoring": {},
                "niveles_abstraccion": {
                    "policy_areas": [{"policy_area_id": "PA01", "dimension_ids": ["DIM01"]}],
    ...
    def test_insufficient_coverage_below_threshold(self):
        ...
        aggregator = DimensionAggregator(monolith=None, abort_on_insufficient=True)
...
class TestPhase4AdversarialStress:
    ...
    def test_many_results_beyond_expected(self):
        ...
        results = [create_valid_scored_result(question_id=i, score=2.0) for i in range(1, 11)]
