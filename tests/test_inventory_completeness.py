...
Validates the methods_inventory_raw.json file generated by scan_methods_inventory.py.
This test suite ensures proper scanner operation and inventory completeness.

See METHODS_INVENTORY_README.md for details on the scanner workflow and integration.
"""
...
def test_minimum_method_count(inventory):
    ...
    assert (
        total >= MINIMUM_METHOD_COUNT
    ), f"Insufficient methods: {total} < {MINIMUM_METHOD_COUNT}"
...
def test_canonical_identifier_format(inventory):
    ...
    for method in methods:
        ...
        assert (
            len(parts) >= MIN_CANONICAL_ID_PARTS
        ), f"Invalid canonical ID format: {cid}"

        if method["class_name"]:
            assert (
                len(parts) >= MIN_CLASS_METHOD_PARTS
            ), f"Class method must have at least {MIN_CLASS_METHOD_PARTS} parts: {cid}"
...
def test_epistemology_tags_present(inventory):
    ...
    assert tag_ratio > MIN_TAG_RATIO, f"Too few methods tagged: {tag_ratio:.2%}"
...
def test_derek_beach_methods_complete(inventory):
    ...
    assert (
        len(derek_methods) > MIN_DEREK_BEACH_METHODS
    ), f"Too few derek_beach methods: {len(derek_methods)}"
...
def test_aggregation_classes_present(inventory):
    ...
    for req_class in required_classes:
        assert (
            req_class in found_classes
        ), f"Required aggregation class not found: {req_class}"
...
def test_executor_methods_present(inventory):
    ...
    assert (
        len(executor_methods) > MIN_EXECUTOR_METHODS
    ), f"Too few executor methods: {len(executor_methods)}"
