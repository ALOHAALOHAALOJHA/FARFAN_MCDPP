...
class TestChoquetBoundedness:
    ...
    def test_boundedness_uniform_weights(self, scores: dict[str, float]) -> None:
        ...
        result = aggregator.aggregate(subject="test_subject", layer_scores=scores)

        assert (
            0.0 <= result.calibration_score <= 1.0
    ...
    def test_boundedness_with_interactions(
        self, scores: dict[str, float], interaction_weight: float
    ) -> None:
        """Cal(I) must be in [0,1] with interaction terms (after clamping)."""
        config = ChoquetConfig(
            linear_weights={"@b": 0.4, "@chain": 0.3, "@q": 0.3},
            interaction_weights={("@b", "@chain"): interaction_weight},
    ...
    def test_boundedness_random_config(self, n_layers: int, n_interactions: int) -> None:
        """Cal(I) bounded for random layer/interaction configurations."""
        layer_ids = [f"@layer{i}" for i in range(n_layers)]

        linear_weights = {layer: 1.0 / n_layers for layer in layer_ids}
        ...
        aggregator = ChoquetAggregator(config)
        result = aggregator.aggregate(subject="test_subject", layer_scores=layer_scores)
...
class TestChoquetMonotonicity:
    ...
    @settings(max_examples=100, deadline=None)
    def test_monotonicity_single_layer(self, base_score: float, increment: float) -> None:
    ...
    @settings(max_examples=100, deadline=None)
    def test_monotonicity_all_layers(self, scores: dict[str, float], scale_factor: float) -> None:
        """Scaling all scores up increases Cal(I)."""
        scaled_scores = {layer: min(1.0, score * scale_factor) for layer, score in scores.items()}

        assume(any(scaled_scores[layer] > scores[layer] for layer in scores))
...
class TestChoquetNormalization:
    ...
    def test_normalization_property(self, weights: dict[str, float]) -> None:
        ...
        config = ChoquetConfig(
            linear_weights=weights, interaction_weights={}, normalize_weights=True
        )
...
class TestChoquetInteractionTerms:
    ...
    @settings(max_examples=100, deadline=None)
    def test_interaction_symmetry(self, score_b: float, score_chain: float) -> None:
...
class TestChoquetDeterminism:
    ...
    def test_deterministic_output(self, scores: dict[str, float]) -> None:
        ...
        assert (
            result1.breakdown.interaction_contribution == result2.breakdown.interaction_contribution
        )
...
class TestChoquetConfigValidation:
    ...
    def test_interaction_missing_layer_raises(self) -> None:
        ...
        with pytest.raises(CalibrationConfigError, match="not in linear_weights"):
            ChoquetConfig(linear_weights={"@b": 0.5}, interaction_weights={("@b", "@missing"): 0.1})
...
class TestChoquetBreakdown:
    ...
    def test_breakdown_components_sum_to_total(self) -> None:
        ...
        total_from_breakdown = (
            result.breakdown.linear_contribution + result.breakdown.interaction_contribution
        )
