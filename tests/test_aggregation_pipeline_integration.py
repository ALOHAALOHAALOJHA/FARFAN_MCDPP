...
class TestFullAggregationPipelineIntegration:
    ...
    def test_full_pipeline_with_real_monolith(self, real_monolith, synthetic_phase3_output):
        ...
        assert len(dimension_scores) == 60, f"Expected 60 dimensions (6×10), got {len(dimension_scores)}"
        
        # Verify traceability
        sample_dim = dimension_scores[0]
        assert len(sample_dim.contributing_questions) > 0, "Dimension score not traceable to micro questions"
        ...
        assert macro_score.score > 0, f"Macro score is ZERO despite valid inputs: {macro_score.score}"
    ...
    def test_empty_phase4_output_fails_validation(self, real_monolith):
        ...
        """
        from canonic_phases.Phase_four_five_six_seven.aggregation_validation import validate_phase4_output
    ...
    def test_score_traceability_chain(self, real_monolith, synthetic_phase3_output):
        ...
        dimension_scores = dim_agg.run(synthetic_phase3_output, group_by_keys=settings.dimension_group_by_keys)
        ...
        assert len(dim_sample.contributing_questions) > 0, "Dimension not traceable to micro questions"
        ...
        cluster_with_areas = next(cs for cs in cluster_scores if cs.cluster_id == first_cluster.cluster_id)
    ...
    def test_aggregation_with_mixed_quality_scores(self, real_monolith):
        ...
        dimension_scores = dim_agg.run(scored_results, group_by_keys=settings.dimension_group_by_keys)
...
class TestAggregationEdgeCases:
    ...
    def test_aggregation_with_boundary_scores(self, real_monolith):
        ...
        dimension_scores_min = dim_agg.run(scored_results_min, group_by_keys=settings.dimension_group_by_keys)
        ...
        dimension_scores_max = dim_agg.run(scored_results_max, group_by_keys=settings.dimension_group_by_keys)
        
        assert len(dimension_scores_max) == 1
        assert abs(dimension_scores_max[0].score - 3.0) < 1e-10, "Aggregation of max scores should yield max"
