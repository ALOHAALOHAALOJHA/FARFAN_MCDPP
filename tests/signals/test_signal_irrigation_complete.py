...
def test_scoring_modality_coverage(signal_registry):
    ...
    assert not missing_modality, (
        f"{len(missing_modality)} questions missing scoring_modality: "
        f"{missing_modality[:10]}"
    )
...
def test_dimension_coverage(signal_registry):
    ...
    assert not missing_dimension, (
        f"{len(missing_dimension)} questions missing dimension_id: "
        f"{missing_dimension[:10]}"
    )
...
def test_critical_questions_fully_irrigated(signal_registry, question_id):
    ...
    assert signals.modality == signals.scoring_modality, (
        f"{question_id}: modality mismatch"
    )
