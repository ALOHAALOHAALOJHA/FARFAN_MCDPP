...
from farfan_pipeline.phases.Phase_one.phase1_09_00_circuit_breaker import (
...
class TestPhase1CircuitBreaker:
    ...
    def test_insufficient_disk_detection(self, mock_disk):
        ...
        mock_disk.return_value = MagicMock(
            free=100 * 1024 * 1024, total=100 * 1024 * 1024 * 1024  # 100 MB  # 100 GB
        )
...
class TestSubphaseCheckpoint:
    ...
    def test_checkpoint_validation_pass(self):
        ...
        passed, errors = checkpoint.validate_checkpoint(
            subphase_num=1, output=mock_output, expected_type=list, validators=validators
        )
    ...
    def test_checkpoint_validation_fail_type(self):
        ...
        passed, errors = checkpoint.validate_checkpoint(
            subphase_num=1, output=mock_output, expected_type=list, validators=[]
        )
    ...
    def test_checkpoint_records_metadata(self):
        ...
        checkpoint.validate_checkpoint(
            subphase_num=5, output=mock_output, expected_type=list, validators=[]
        )
...
class TestDependencyCheck:
    ...
    def test_dependency_check_available(self):
        ...
        check = DependencyCheck(
            name="test_package", available=True, version="1.0.0", severity=FailureSeverity.CRITICAL
        )
...
class TestResourceCheck:
    ...
    def test_resource_check_sufficient(self):
        ...
        check = ResourceCheck(
            resource_type="memory", available=8.0, required=2.0, sufficient=True, unit="GB"
        )
    ...
    def test_resource_check_insufficient(self):
        ...
        check = ResourceCheck(
            resource_type="disk", available=0.5, required=1.0, sufficient=False, unit="GB"
        )
